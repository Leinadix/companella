...
public partial class SkillsOverTimeChart : CompositeDrawable
{
	...
	private const float _trendLineThickness = 3f;
	private const int _movingAverageWindow = 15;
	...
	private const float _zoomStep = 0.15f;
	...
	private float _viewOffset; // 0 = start, 1-viewWidth = end
	...
	protected override bool OnScroll(ScrollEvent e)
	{
		...
		var relativeX = (mousePos.X - chartBounds.Left) / chartBounds.Width;
		relativeX = Math.Clamp(relativeX, 0, 1);

		// Calculate the raw data position under the mouse before zoom
		var viewWidth = 1f / _zoomLevel;
		var rawXUnderMouse = relativeX * viewWidth + _viewOffset;

		// Apply zoom
		if (e.ScrollDelta.Y > 0)
			_zoomLevel = Math.Min(_maxZoom, _zoomLevel * (1 + _zoomStep));
		else
			_zoomLevel = Math.Max(_minZoom, _zoomLevel * (1 - _zoomStep));

		// Adjust offset to keep the same data point under the mouse
		var newViewWidth = 1f / _zoomLevel;
	...
	protected override bool OnDragStart(DragStartEvent e)
	{
		...
		{
			var windowPos = GameHost.Window.Position;
		...
		if (GameHost.Window != null)
			GameHost.Window.CursorState |= CursorState.Hidden | CursorState.Confined;
	...
	protected override void OnDrag(DragEvent e)
	{
		...
		var deltaX = cursorPos.X - _dragCenterScreen.X;
		...
		var chartWidth = _pointsContainer.DrawSize.X;
		var viewWidth = 1f / _zoomLevel;

		// Convert pixel delta to data units (deltaX is in screen pixels, so scale appropriately)
		var scaleFactor = chartWidth / _chartArea.DrawRectangle.Width;
		var localDeltaX = deltaX * scaleFactor;
		var deltaData = localDeltaX / chartWidth * viewWidth * -1;

		// Calculate new offset (but don't apply yet)
		var newViewOffset = _viewOffset + deltaData;
		var maxOffset = Math.Max(0, 1f - viewWidth);
		newViewOffset = Math.Clamp(newViewOffset, 0, maxOffset);

		// Calculate how far we'd be from rendered center with new offset
		var newViewCenter = newViewOffset + viewWidth / 2f;
		var distanceFromRenderCenter = Math.Abs(newViewCenter - _renderedCenterOffset);
		var distancePercent = distanceFromRenderCenter / viewWidth;
		...
		var offsetFromRender = _viewOffset - _renderedViewOffset;
		var visualOffsetPx = -offsetFromRender / viewWidth * chartWidth;
	...
	private void UpdateSkillsetVisibility(bool animate = true)
	{
		...
		foreach (var (key, drawables) in _skillsetDrawables)
		{
			...
			if (key.StartsWith("_acc_", StringComparison.OrdinalIgnoreCase))
			{
	...
	private float GetSkillsetAlpha(string skillset)
	{
		...
		if (_hoveredSkillset != null) return skillset == _hoveredSkillset ? 1f : 0.1f;

		// If there are selected skillsets, only those are visible
		if (_selectedSkillsets.Count > 0) return _selectedSkillsets.Contains(skillset) ? 1f : 0.1f;
	...
	private void Redraw()
	{
		...
		UpdateSkillsetVisibility(false);
	}
	...
	private void CalculateRanges()
	{
		...
		if (_endTime <= _startTime) _endTime = _startTime.AddDays(1);
	}
	...
	{
		if (_sortedPlays.Count <= 1)
			return 0.5f;

		// Raw position (0-1) based on index
		var rawX = (float)index / (_sortedPlays.Count - 1);

		// Apply zoom and pan transformation
		var viewWidth = 1f / _zoomLevel;
		var visibleX = (rawX - _viewOffset) / viewWidth;
	...
	private int GetIndexAtVisibleX(float visibleX)
	{
		...
		var viewWidth = 1f / _zoomLevel;
		var rawX = visibleX * viewWidth + _viewOffset;

		var index = (int)Math.Round(rawX * (_sortedPlays.Count - 1));
	...
	private void DrawAxisLabels()
	{
		...
		for (var i = 0; i <= 4; i++)
		{
			var value = _accMax - (_accMax - _accMin) * (i / 4f);
			var y = _chartPaddingTop + (DrawHeight - _chartPaddingTop - _chartPaddingBottom) * (i / 4f);
		...
		for (var i = 0; i <= timeLines; i++)
		{
			...
			if (_sortedPlays.Count > 0)
			{
				...
				var playIndex = GetIndexAtVisibleX(i / (float)timeLines);
			...
			_labelsContainer.Add(new SpriteText
			{
				Text = time.ToLocalTime().ToString(format, CultureInfo.InvariantCulture),
	...
	{
		if (_trendData == null || _sortedPlays.Count == 0)
			return;

		var totalCount = _sortedPlays.Count;
		var viewWidth = 1f / _zoomLevel;

		// Calculate buffered render range: 3x visible width centered on _renderedCenterOffset
		var renderStart = Math.Max(0, _renderedCenterOffset - 1.5f * viewWidth);
		var renderEnd = Math.Min(1f, _renderedCenterOffset + 1.5f * viewWidth);

		// Convert to index range
		var startIdx = (int)Math.Floor(renderStart * (totalCount - 1));
		var endIdx = (int)Math.Ceiling(renderEnd * (totalCount - 1));
		startIdx = Math.Max(0, startIdx);
		endIdx = Math.Min(totalCount - 1, endIdx);
		var renderCount = endIdx - startIdx + 1;

		// Calculate downsampling step based on render range and zoom
		var targetPoints = (int)(_maxVisiblePoints * Math.Max(1f, _zoomLevel));
		var step = Math.Max(1, renderCount / targetPoints);
		...
		var drawLines = renderCount / step <= _maxVisiblePoints * 2;
		if (drawLines)
			foreach (var (skillset, points) in pointsBySkillset)
			{
				...
				for (var i = 1; i < points.Count; i++)
					skillsetContainer.Add(new SkillsLineSegment(points[i - 1], points[i], color, _lineThickness * 0.7f));
			}
	}

	private Container GetOrCreateSkillsetContainer(string skillset)
	{
		_skillsetDrawables.TryGetValue(skillset, out var drawables);
		if (drawables == null)
		{
	...
	{
		if (_trendData == null || _sortedPlays.Count < 3)
			return;
		...
		foreach (var (skillset, plays) in playsBySkillset)
		{
			...
			if (plays.Count >= 3) DrawRegressionLine(plays, color, skillset);
		}
	}
	...
	private void DrawMovingAverageLine(List<(int index, float msd)> plays, Color4 color, string skillset)
	{
		...
		var step = Math.Max(1, plays.Count / 100);

		var movingAvgPoints = new List<Vector2>();
		var windowSize = Math.Min(_movingAverageWindow, Math.Max(3, plays.Count / 5));

		for (var i = 0; i < plays.Count; i += step)
		{
			// Calculate moving average centered on current point
			var start = Math.Max(0, i - windowSize / 2);
			var end = Math.Min(plays.Count - 1, i + windowSize / 2);
		...
		for (var i = 1; i < movingAvgPoints.Count; i++)
			skillsetContainer.Add(new SkillsLineSegment(movingAvgPoints[i - 1], movingAvgPoints[i], trendColor,
				_trendLineThickness));
	}
	...
	private void DrawRegressionLine(List<(int index, float msd)> plays, Color4 color, string skillset)
	{
		...
		var startIndex = plays[0].index;
		var endIndex = plays[^1].index;

		var startMsd = (float)(slope * startIndex + intercept);
		var endMsd = (float)(slope * endIndex + intercept);

		var x1 = GetXPositionByIndex(startIndex);
		var y1 = 1f - (startMsd - _msdMin) / (_msdMax - _msdMin);
		var x2 = GetXPositionByIndex(endIndex);
		var y2 = 1f - (endMsd - _msdMin) / (_msdMax - _msdMin);
		...
		DrawDashedLine(skillsetContainer, new Vector2(x1, y1), new Vector2(x2, y2), regressionColor,
			_lineThickness * 1.5f);
	}

	private static void DrawDashedLine(Container container, Vector2 from, Vector2 to, Color4 color, float thickness)
	{
		...
		while (currentPos < totalLength)
		{
			...
			var endPos = Math.Min(currentPos + segmentLength, totalLength);
	...
	private void DrawAccuracyData()
	{
		...
		foreach (var (skillset, points) in skillsetAccPoints)
		{
			...
			foreach (var (_, point) in points)
				...
				{
					Size = new Vector2(_pointRadius * 1.5f),
					RelativePositionAxes = Axes.Both,
					Position = point,
					Origin = Anchor.Centre,
					Colour = accColor
			...
			if (points.Count <= _maxVisiblePoints * 2)
			{
				var lineColor = new Color4(accColor.R, accColor.G, accColor.B, (byte)120);
				for (var i = 1; i < points.Count; i++)
					accContainer.Add(new SkillsLineSegment(points[i - 1].point, points[i].point, lineColor,
						_lineThickness * 0.5f));
			}
		}
	}
