...
public partial class TimingDeviationChart : CompositeDrawable
{
	...
	private const float _leftPanelWidth = 0.22f; // Left bar chart takes 22% of width
	private const float _rightPanelPadding = 0.01f; // Small right padding
	private const float _topPadding = 0.12f; // Top area for title (increased)
	private const float _bottomPadding = 0.02f; // Small bottom padding
	private const float _bellCurveRelativeHeight = 0.42f; // Bell curve takes 42% of height
	private const float _keyIndicatorRelativeHeight = 0.08f; // Key buttons take 8%
	private const float _pointRadius = 3f;

	// Deviation range in ms (will be auto-scaled)
	private const float _defaultDeviationRange = 100f;

	// Colors for different judgements
	private static readonly Color4 _colorMax300 = new(200, 200, 255, 255); // Light blue (rainbow)
	private static readonly Color4 _color300 = new(100, 180, 255, 255); // Blue
	private static readonly Color4 _color200 = new(100, 220, 100, 255); // Green
	private static readonly Color4 _color100 = new(255, 220, 100, 255); // Yellow
	private static readonly Color4 _color50 = new(255, 150, 80, 255); // Orange
	private static readonly Color4 _colorMiss = new(255, 80, 80, 255); // Red

	// Colors for columns/keys (for key indicators)
	private static readonly Color4[] _columnColors =
	{
		new(255, 100, 100, 255), // Column 0 - Red
		new(100, 200, 255, 255), // Column 1 - Cyan
		new(255, 220, 100, 255), // Column 2 - Yellow
		new(100, 255, 150, 255), // Column 3 - Green
		new(200, 150, 255, 255), // Column 4 - Purple
		new(255, 180, 100, 255), // Column 5 - Orange
		new(150, 220, 255, 255) // Column 6 - Light Blue
	};
	...
	private float _deviationMax = _defaultDeviationRange;
	...
	private bool _isManiaV2; // osu!mania v1 vs v2 scoring
	...
	public float TimeSelectionStart
	{
		...
		set => _timeSelectionStart = Math.Clamp(value, 0f, 1f);
	}
	...
	public float TimeSelectionEnd
	{
		...
		set => _timeSelectionEnd = Math.Clamp(value, 0f, 1f);
	}
	...
	private void load()
	{
		...
		InternalChildren = new Drawable[]
		{
			...
			{
				RelativeSizeAxes = Axes.Both,
				Width = 1f - _leftPanelWidth - _rightPanelPadding,
				Height = _keyIndicatorRelativeHeight,
				RelativePositionAxes = Axes.Both,
				X = _leftPanelWidth,
				Y = 1f - _keyIndicatorRelativeHeight,
			...
			_columnStatsContainer = new Container
			{
				RelativeSizeAxes = Axes.Both,
				Width = 0.12f,
				Height = 1f - _topPadding - _bellCurveRelativeHeight - _keyIndicatorRelativeHeight - _bottomPadding,
				RelativePositionAxes = Axes.Both,
				X = 1f - 0.12f - _rightPanelPadding,
				Y = _topPadding,
				Anchor = Anchor.TopLeft,
				Origin = Anchor.TopLeft,
				Alpha = 0
			...
			{
				RelativeSizeAxes = Axes.Both,
				Width = _leftPanelWidth - 0.01f,
				Height = 1f - _topPadding - _keyIndicatorRelativeHeight - 0.02f,
				RelativePositionAxes = Axes.Both,
				X = 0.005f,
				Y = _topPadding,
				Anchor = Anchor.TopLeft,
			...
			_accuracyText = new SpriteText
			{
				...
				RelativePositionAxes = Axes.Both,
				X = 0.01f,
				Y = 0.99f - _keyIndicatorRelativeHeight,
				Anchor = Anchor.TopLeft,
			...
			_manipFactorText = new SpriteText
			{
				...
				RelativePositionAxes = Axes.Both,
				X = 0.01f,
				Y = 1.05f - _keyIndicatorRelativeHeight,
				Anchor = Anchor.TopLeft,
				Origin = Anchor.BottomLeft
			},
			...
			_bellCurveContainer = new Container
			{
				RelativeSizeAxes = Axes.Both,
				Width = 1f - _leftPanelWidth - _rightPanelPadding,
				Height = _bellCurveRelativeHeight,
				RelativePositionAxes = Axes.Both,
				X = _leftPanelWidth,
				Y = 1f - _bellCurveRelativeHeight - _keyIndicatorRelativeHeight,
				Anchor = Anchor.TopLeft,
				Origin = Anchor.TopLeft,
				Alpha = 0
			},
					...
					_timeSelectionContainer = new Container
					{
						...
						Children = new Drawable[]
						{
							...
							new DraggableTimeHandle(
								true,
								(delta) => OnTimeEdgeDrag(true, delta),
								() =>
								{
									RedrawSelectionDependentCharts();
									TimeSelectionChanged?.Invoke();
								}
							)
							{
							...
							new DraggableTimeHandle(
								false,
								(delta) => OnTimeEdgeDrag(false, delta),
								() =>
								{
									RedrawSelectionDependentCharts();
									TimeSelectionChanged?.Invoke();
	...
	{
		if (column < 0 || column >= _activeColumns.Length)
			return false;
	...
	{
		if (_data == null)
			return new List<TimingDeviation>();
	...
	private static ManiaJudgement GetManiaJudgement(double absDeviation, int od)
	{
		...
		if (absDeviation <= window300)
			return ManiaJudgement.Hit300;
		if (absDeviation <= window200)
			return ManiaJudgement.Hit200;
		if (absDeviation <= window100)
			return ManiaJudgement.Hit100;
		if (absDeviation <= window50)
	...
	private static ManiaJudgement GetEtternaJudgement(double absDeviation, int judge)
	{
		...
		if (absDeviation <= marvelous)
			return ManiaJudgement.Max300; // Marvelous
		if (absDeviation <= perfect)
			return ManiaJudgement.Hit300; // Perfect
		if (absDeviation <= great)
			return ManiaJudgement.Hit200; // Great
		if (absDeviation <= good)
			return ManiaJudgement.Hit100; // Good
		if (absDeviation <= boo)
			return ManiaJudgement.Hit50; // Boo
		return ManiaJudgement.Miss;
	}
	...
	private void DrawDistributionChart()
	{
		...
		var startTime = _timeSelectionStart * _data.MapDuration;
		var endTime = _timeSelectionEnd * _data.MapDuration;
		...
		foreach (var deviation in selectedPoints) counts[deviation.Judgement]++;

		var totalHits = selectedPoints.Count;
		...
		var accuracy = CalculateAccuracy(counts, totalHits, selectedPoints);
		...
		var manipResult = ManipFactorCalculator.Calculate(selectedPoints, _keyCount);
		...
		var containerWidth = _distributionContainer.DrawWidth;
		var containerHeight = _distributionContainer.DrawHeight;
		...
		var judgements = new[]
		{
			(ManiaJudgement.Max300, "MAX", _colorMax300),
			(ManiaJudgement.Hit300, "300", _color300),
			(ManiaJudgement.Hit200, "200", _color200),
			(ManiaJudgement.Hit100, "100", _color100),
			(ManiaJudgement.Hit50, "50", _color50),
			(ManiaJudgement.Miss, "Miss", _colorMiss)
		};

		var labelWidthRel = 0.20f; // 20% for labels
		var countWidthRel = 0.20f; // 20% for count
		var barWidthRel = 1f - labelWidthRel - countWidthRel - 0.02f; // Rest for bar

		float numBars = judgements.Length;
		var headerSpace = 0.07f; // Space for header
		var totalBarArea = 0.80f; // Use 80% of height for bars
		var barHeightRel = totalBarArea / numBars * 0.65f; // Each bar
		var barSpacingRel = totalBarArea / numBars * 0.35f; // Spacing
		var startY = headerSpace; // Start after header

		var yOffset = startY;

		foreach (var (judgement, label, color) in judgements)
		{
			var count = counts[judgement];
			var barFillRel = maxCount > 0 ? count / (float)maxCount * barWidthRel : 0;

			var barHeight = barHeightRel * containerHeight;
			var yPos = yOffset * containerHeight;
	...
	private void DrawBellCurve()
	{
		...
		var mean = selectedPoints.Average(d => d.Deviation);
		double stdDev = 0;
		if (selectedPoints.Count > 1)
		{
			var sumSquaredDiff = selectedPoints.Sum(d => Math.Pow(d.Deviation - mean, 2));
		...
		var containerWidth = _bellCurveContainer.DrawWidth;
		var containerHeight = _bellCurveContainer.DrawHeight;
		var headerPadding = 18f; // Space for header
		var footerPadding = 14f; // Space for labels at bottom
		var sidePadding = 10f; // Side padding
		var chartWidth = containerWidth - sidePadding * 2;
		var chartHeight = containerHeight - headerPadding - footerPadding;
		var topBoost = 8f; // Extra space at top for visual scaling

		// Use the graph's deviation range for the bell curve X-axis
		var deviationMin = -_deviationMax;
		var deviationMax = _deviationMax;
		var boundsRange = deviationMax - deviationMin;
		...
		var headerText = selectionPercent < 99.9
		...
		var numPoints = 80;
		var density = new float[numPoints];

		// Bandwidth for KDE (Silverman's rule of thumb)
		var bandwidth = 1.06 * stdDev * Math.Pow(selectedPoints.Count, -0.2);
		if (bandwidth < 1)
			bandwidth = 1; // Minimum bandwidth of 1ms

		// Calculate density at each point using Gaussian kernel
		for (var i = 0; i < numPoints; i++)
		{
			var x = deviationMin + i / (float)(numPoints - 1) * boundsRange;
			double sum = 0;

			foreach (var dev in selectedPoints)
			{
				var u = (x - dev.Deviation) / bandwidth;
		...
		var maxDensity = density.Max();
		if (maxDensity < 0.0001f)
			maxDensity = 0.0001f;

		// Draw filled area under curve (as vertical bars for smooth fill)
		// Bars are centered on their X position
		var barWidth = chartWidth / numPoints + 1;
		var curveAreaHeight = chartHeight - topBoost;
		var curveTop = headerPadding + topBoost;

		for (var i = 0; i < numPoints; i++)
		{
			var x = sidePadding + i / (float)(numPoints - 1) * chartWidth;
			var normalizedHeight = density[i] / maxDensity;
			var barHeight = normalizedHeight * curveAreaHeight;

			if (barHeight > 0.5f)
			{
				var devValue = deviationMin + i / (float)(numPoints - 1) * boundsRange;
		...
		{
			var x1 = sidePadding + i / (float)(numPoints - 1) * chartWidth;
			var x2 = sidePadding + (i + 1) / (float)(numPoints - 1) * chartWidth;
			// Y coordinates: bottom of curve area minus the normalized height
			var y1 = containerHeight - footerPadding - density[i] / maxDensity * curveAreaHeight;
			var y2 = containerHeight - footerPadding - density[i + 1] / maxDensity * curveAreaHeight;

			var devValue = deviationMin + (i + 0.5f) / (float)(numPoints - 1) * boundsRange;
			var color = GetDeviationColor(Math.Abs(devValue));
		...
		// Draw zero line
		var zeroX = sidePadding + (0 - deviationMin) / boundsRange * chartWidth;
		_bellCurveContainer.Add(new Box
		{
		...
		// Draw mean line
		var meanX = sidePadding + (float)(mean - deviationMin) / boundsRange * chartWidth;
		if (meanX >= sidePadding && meanX <= sidePadding + chartWidth)
		{
			...
			var meanLabel = mean >= 0 ? $"+{mean:F1}" : $"{mean:F1}";
			_bellCurveContainer.Add(new SpriteText
			{
				Text = meanLabel,
	...
	private Color4 GetDeviationColor(float absDeviation)
	{
		...
		if (absDeviation <= windows.marvelous)
			return _colorMax300;
		if (absDeviation <= windows.perfect)
			return _color300;
		if (absDeviation <= windows.great)
			return _color200;
		if (absDeviation <= windows.good)
			return _color100;
		if (absDeviation <= windows.boo)
			return _color50;
		return _colorMiss;
	}
	...
	private void DrawTimeSelection()
	{
		...
		var selectionWidth = Math.Max(0.05f, _timeSelectionEnd - _timeSelectionStart);
	...
	{
		var chartWidth = _chartArea.DrawWidth;
		if (chartWidth <= 0)
			return;

		// Convert pixel delta to normalized delta
		var normalizedDelta = deltaX / chartWidth;

		if (isLeftEdge)
		{
			// Left edge controls start time
			var newStart = _timeSelectionStart + normalizedDelta;
			// Ensure minimum range and stay within bounds
			if (newStart >= 0 && newStart < _timeSelectionEnd - _minTimeSelectionRange)
			{
		...
		else
		{
			...
			var newEnd = _timeSelectionEnd + normalizedDelta;
			// Ensure minimum range and stay within bounds
			if (newEnd <= 1 && newEnd > _timeSelectionStart + _minTimeSelectionRange)
			{
	...
	private double CalculateAccuracy(Dictionary<ManiaJudgement, int> counts, int totalHits,
		List<TimingDeviation>? deviations = null)
	{
		if (totalHits == 0)
			return 0;
		...
		if (_hitWindowSystemType == HitWindowSystemType.OsuMania)
		{
			if (_isManiaV2)
			{
				...
				var score = counts[ManiaJudgement.Max300] * 305.0
			...
			else
			{
				...
				var score = counts[ManiaJudgement.Max300] * 300.0
							+ counts[ManiaJudgement.Hit300] * 300.0
							+ counts[ManiaJudgement.Hit200] * 200.0
							+ counts[ManiaJudgement.Hit100] * 100.0
							+ counts[ManiaJudgement.Hit50] * 50.0;
		...
		else
		{
			...
			if (deviations != null && deviations.Count > 0)
			{
				...
				var maxScore = deviations.Count * 100.0;
			...
			else
			{
				...
				var wifeScore = counts[ManiaJudgement.Max300] * 100.0 // Marvelous
	...
	private static double CalculateWife3Points(double deviationMs, int judge)
	{
		...
		var booWindow = 180.0 * judgeScale;

		// Take absolute deviation
		var absDeviation = Math.Abs(deviationMs);
		...
		var marvelousWindow = 22.5 * judgeScale;

		// Normalize deviation to the marvelous window scale
		var normalizedDev = absDeviation / marvelousWindow;
		...
		var sigma = booWindow / 2.5; // Controls the steepness of the falloff
		var x = (booWindow - absDeviation) / sigma;
	...
	{
		if (accuracy >= 99)
			return new Color4(200, 200, 255, 255); // Light blue (SS)
		if (accuracy >= 95)
			return new Color4(100, 220, 100, 255); // Green (S)
		if (accuracy >= 90)
			return new Color4(180, 220, 100, 255); // Yellow-green (A)
		if (accuracy >= 80)
			return new Color4(255, 220, 100, 255); // Yellow (B)
		if (accuracy >= 70)
			return new Color4(255, 150, 80, 255); // Orange (C)
	...
	private void UpdateStatsText()
	{
		...
		var meanDev = filtered.Average(d => d.Deviation);
		var sumSquaredDiff = filtered.Sum(d => Math.Pow(d.Deviation - meanDev, 2));
		var stdDev = Math.Sqrt(sumSquaredDiff / filtered.Count);
		var ur = stdDev * 10;
	...
	{
		var containerWidth = _keyIndicatorsContainer.DrawWidth;
		var containerHeight = _keyIndicatorsContainer.DrawHeight;

		// Button sizes relative to container
		var buttonWidthRel = 0.08f; // Each button 8% of container width
		var buttonSpacingRel = 0.015f; // 1.5% spacing
		var totalWidthRel = _keyCount * buttonWidthRel + (_keyCount - 1) * buttonSpacingRel;
		var startXRel = (1f - totalWidthRel) / 2; // Center the buttons

		for (var i = 0; i < _keyCount; i++)
		{
			var column = i;
			var isActive = _activeColumns[column];
			var color = isActive ? _columnColors[column % _columnColors.Length] : new Color4(60, 60, 70, 255);
			...
			var keyButton = new ClickableKeyButton(column, isActive, color, () => ToggleColumn(column))
			{
				...
				Position = new Vector2((startXRel + i * (buttonWidthRel + buttonSpacingRel)) * containerWidth,
	...
	private partial class ClickableKeyButton : CompositeDrawable
	{
		...
		protected override bool OnHover(HoverEvent e)
		{
			_background.FadeColour(_isActive
				? new Color4((byte)Math.Min(_color.R + 30, 255), (byte)Math.Min(_color.G + 30, 255),
					(byte)Math.Min(_color.B + 30, 255), 200)
	...
	private void DrawColumnStats()
	{
		...
		for (var i = 0; i < _keyCount; i++)
		{
			...
			var isActive = _activeColumns[i];
			var color = isActive ? _columnColors[i % _columnColors.Length] : new Color4(80, 80, 80, 255);
			...
			{
				var mean = columnDeviations.Average(d => d.Deviation);
				var sumSq = columnDeviations.Sum(d => Math.Pow(d.Deviation - mean, 2));
				var stdDev = Math.Sqrt(sumSq / columnDeviations.Count);
				var ur = stdDev * 10;
	...
	private void DrawGrid()
	{
		...
		DrawJudgementLine(windows.marvelous, _colorMax300, "Marvelous");
		DrawJudgementLine(windows.perfect, _color300, "Perfect");
		DrawJudgementLine(windows.great, _color200, "Great");
		DrawJudgementLine(windows.good, _color100, "Good");
		DrawJudgementLine(windows.boo, _color50, "Boo");

		// Vertical grid lines (time divisions)
		if (_data != null && _data.MapDuration > 0)
		{
			var numDivisions = Math.Max(4, Math.Min(10, (int)(_data.MapDuration / 30000))); // One per ~30 seconds
			for (var i = 0; i <= numDivisions; i++)
			{
				var x = i / (float)numDivisions;
	...
	{
		if (deviationMs > _deviationMax)
			return; // Don't draw if outside visible range

		var normalizedDev = deviationMs / _deviationMax;

		// Line above zero (early hits - negative deviation shown as positive Y)
		var yAbove = 0.5f - normalizedDev * 0.5f;
		// Line below zero (late hits - positive deviation)
		var yBelow = 0.5f + normalizedDev * 0.5f;
	...
	private void DrawAxisLabels()
	{
		...
		var chartTop = _topPadding;
		var chartBottom = 1f - _bellCurveRelativeHeight - _keyIndicatorRelativeHeight - _bottomPadding;
		var chartHeight = chartBottom - chartTop;
		var chartLeft = _leftPanelWidth;
		...
		{
			var relY = chartTop + chartHeight * (0.5f - level * 0.5f);
			var value = level * _deviationMax;

			var label = level == 0 ? "0" : $"{value:+0;-0}";
		...
		if (_data != null && _data.MapDuration > 0)
		{
			var chartWidth = 1f - _leftPanelWidth - _rightPanelPadding;
			var numDivisions = Math.Max(4, Math.Min(10, (int)(_data.MapDuration / 30000)));
			for (var i = 0; i <= numDivisions; i++)
			{
				var relX = _leftPanelWidth + chartWidth * (i / (float)numDivisions);
				var time = TimeSpan.FromMilliseconds(_data.MapDuration * (i / (float)numDivisions));

				_labelsContainer.Add(new SpriteText
				{
					Text = time.ToString(@"m\:ss", CultureInfo.InvariantCulture),
					Font = new FontUsage("", 13),
					Colour = new Color4(150, 150, 150, 255),
					RelativePositionAxes = Axes.Both,
	...
	private void DrawDataPoints()
	{
		...
		foreach (var deviation in filtered)
		{
			...
			var x = (float)(deviation.ExpectedTime / _data.MapDuration);
			x = Math.Clamp(x, 0, 1);

			// Y position based on deviation (inverted: negative at top, positive at bottom)
			// Clamp to range
			var normalizedDev = (float)Math.Clamp(deviation.Deviation, -_deviationMax, _deviationMax);
			var y = 0.5f + normalizedDev / _deviationMax * 0.5f;
			y = Math.Clamp(y, 0, 1);
	...
	private static Color4 GetJudgementColor(ManiaJudgement judgement)
	{
		...
		{
			ManiaJudgement.Max300 => _colorMax300,
			ManiaJudgement.Hit300 => _color300,
			ManiaJudgement.Hit200 => _color200,
			ManiaJudgement.Hit100 => _color100,
			ManiaJudgement.Hit50 => _color50,
			ManiaJudgement.Miss => _colorMiss,
			_ => _color300
		};
	}
}
