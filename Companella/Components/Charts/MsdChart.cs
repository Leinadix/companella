...
{
	private const float _maxMsdValue = 40f;
	...
	private static readonly Dictionary<string, string> _skillsetLabels = new()
	{
		{ "overall", "OVR" },
		{ "stream", "STR" },
		{ "jumpstream", "JS" },
	...
	public void SetScores(SkillsetScores scores, float rate = 1.0f)
	{
		...
		foreach (var (name, value) in skillsets)
		{
			...
			var heightPercent = Math.Clamp(value / _maxMsdValue, 0f, 1f);
			var color = _skillsetColors.GetValueOrDefault(name, Color4.Gray);
			var label = _skillsetLabels.GetValueOrDefault(name, name);
