...
public partial class KeybindConfigPanel : CompositeDrawable
{
	...
	private readonly Color4 _recordingColor = new(255, 150, 150, 255);
	...
	{
		if (_pressedKeys.Count > 0) _keybindButton.SetKeybindText(FormatKeybind(BuildKeybindString(_pressedKeys)));
	}
	...
	private partial class KeybindButton : ClickableContainer, IHasTooltip
	{
		...
		{
			if (_isRecording && OnKeyDownCallback != null) return OnKeyDownCallback(e);
		...
		{
			if (_isRecording) OnKeyUpCallback?.Invoke(e);
		...
		{
			base.OnFocus(e);
			if (_isRecording) _background.Colour = RecordingColor;
		}
		...
		{
			if (!_isRecording) _background.Colour = HoverColor;
		...
		{
			if (!_isRecording) _background.Colour = NormalColor;
