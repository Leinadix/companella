...
{
	private static readonly string _toolsDirectory;
	...
	static ToolPaths()
	{
		...
		_toolsDirectory = Path.Combine(AppContext.BaseDirectory, "tools");
	}

	/// <summary>
	/// Gets the path to the bpm.exe executable.
	/// </summary>
	public static string BpmExe => Path.Combine(_toolsDirectory, "bpm.exe");

	/// <summary>
	/// Gets the path to the msd-calculator-515 executable (MinaCalc 5.15).
	/// </summary>
	public static string MsdCalculator515 => Path.Combine(_toolsDirectory, "msd-calculator-515.exe");

	/// <summary>
	/// Gets the path to the msd-calculator-505 executable (MinaCalc 5.05 legacy).
	/// </summary>
	public static string MsdCalculator505 => Path.Combine(_toolsDirectory, "msd-calculator-505.exe");
	...
	public static string GetMsdCalculator(string version)
	{
		return version == "505" ? MsdCalculator505 : MsdCalculator515;
	}
	...
	/// </summary>
	public static string Directory => _toolsDirectory;
	...
	{
		Logger.Info($"[ToolPaths] Tools directory: {_toolsDirectory}");
		...
		Logger.Info(
			$"[ToolPaths] msd-calculator-515.exe: {(MsdCalculator515Exists ? "Found" : "MISSING")} at {MsdCalculator515}");
		Logger.Info(
			$"[ToolPaths] msd-calculator-505.exe: {(MsdCalculator505Exists ? "Found" : "MISSING")} at {MsdCalculator505}");
	}
	...
	{
		var keys = (int)Math.Round(keyCount);
