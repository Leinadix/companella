...
/// </summary>
public sealed class AptabaseService : IDisposable
{
	private const string _appKey = "A-SH-2513306072";
	private const string _apiEndpoint = "/api/v0/events";
	...
	public AptabaseService()
	{
		...
		var baseUrl = GetBaseUrl(_appKey);
	...
	private async Task TrackEventAsync(string eventName, Dictionary<string, object>? props)
	{
		try
		{
			...
			using var response = await _httpClient.PostAsJsonAsync(_apiEndpoint, events, _jsonOptions);
	...
	private static string GetAppVersion()
	{
		try
		{
			...
			if (File.Exists(versionPath)) return File.ReadAllText(versionPath).Trim();
		}
		catch
		{
