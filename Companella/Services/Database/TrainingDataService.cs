...
{
	private const string _trainingFileName = "training-data.json";
	...
	public TrainingDataService()
	{
		...
		_trainingDataPath = Path.Combine(exeDir, _trainingFileName);
	...
	public async Task LoadAsync()
	{
		...
		try
		{
			...
			var loaded = JsonSerializer.Deserialize<TrainingData>(json, _jsonOptions);
			...
			if (loaded != null)
			{
				...
				IsLoaded = true;
				Logger.Info(
					$"[TrainingData] Loaded {_trainingData.Entries.Count} training entries (v{_trainingData.Version})");
			}
			else
			{
				_trainingData = new TrainingData();
	...
	public async Task SaveAsync()
	{
		try
		{
			var json = JsonSerializer.Serialize(_trainingData, _jsonOptions);
	...
	public void AddEntry(SkillsetScores msdScores, double interludeRating, int danIndex, string? sourcePath = null)
	{
		...
		Logger.Info(
			$"[TrainingData] Added entry: MSD Overall {msdScores.Overall:F2} + Interlude {interludeRating:F2} -> Dan {label} (idx={danIndex})");
	}
	...
	public void AddEntry(MsdSkillsetValues msdValues, double interludeRating, int danIndex, string? sourcePath = null)
	{
		...
		var label = DanLookup.GetLabel(danIndex) ?? "?";
		Logger.Info(
			$"[TrainingData] Added entry: MSD Overall {msdValues.Overall:F2} + Interlude {interludeRating:F2} -> Dan {label} (idx={danIndex})");
	}
	...
	{
		return DanLookup.GetIndex(label);
	}
	...
	/// </summary>
	public static string? GetDanLabel(int index)
	{
		return DanLookup.GetLabel(index);
	}

	/// <summary>
	/// Gets all dan labels in order.
	/// </summary>
	public static IReadOnlyList<string> GetAllDanLabels()
	{
		return DanLookup.Labels;
	}
	...
	public async Task<MergeResult> MergeIntoDansAsync(bool enableExtrapolation = false)
	{
		...
		try
		{
			...
			config = JsonSerializer.Deserialize<DanConfiguration>(json, _jsonOptions);
		...
		try
		{
			var outputJson = JsonSerializer.Serialize(config, _jsonOptions);
	...
	public TrainingStatistics GetStatistics()
	{
		...
		for (var danIndex = 0; danIndex < DanLookup.Count; danIndex++)
		{
			...
			if (count > 0) stats.EntriesPerDan[danIndex] = count;
		}
	...
	/// </summary>
	private static Dictionary<int, (MsdSkillsetValues Msd, double Interlude, int Count)> EnforceMonotonicity(
