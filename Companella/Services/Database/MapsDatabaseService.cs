...
public class MapsDatabaseService : IDisposable
{
	...
	private readonly int _batchWriteSize = 50;
	...
	public MapsDatabaseService()
	{
		...
		if (ToolPaths.MsdCalculatorExists) _msdAnalyzer = new MsdAnalyzer(ToolPaths.MsdCalculator);
	...
	public int Get4KMapCount()
	{
		try
		{
			...
			using var cmd =
				new SqliteCommand("SELECT COUNT(*) FROM Maps WHERE Mode = 3 AND CircleSize = 4", connection);
			return Convert.ToInt32(cmd.ExecuteScalar());
		}
		catch (Exception ex)
		{
	...
	public async Task<bool> IndexBeatmapAsync(string beatmapPath, CancellationToken cancellationToken = default)
	{
		...
		try
		{
			...
			if (existingMap != null && existingMap.FileHash == fileHash) return true; // Already indexed and up to date
			...
			if (isSupportedMania && _msdAnalyzer != null)
				...
				{
					var msdResult =
						await _msdAnalyzer.AnalyzeAsync(beatmapPath, new MsdAnalysisOptions { TimeoutMs = 30000 });
	...
	public List<IndexedMap> SearchMaps(MapSearchCriteria criteria)
	{
		...
		try
		{
			...
			var orderBy = criteria.OrderBy switch
			{
				...
				MapSearchOrderBy.PlayCount =>
					"(SELECT COUNT(*) FROM MapPlayerStats WHERE MapPlayerStats.BeatmapPath = Maps.BeatmapPath)",
				MapSearchOrderBy.BestAccuracy =>
					"(SELECT MAX(Accuracy) FROM MapPlayerStats WHERE MapPlayerStats.BeatmapPath = Maps.BeatmapPath)",
			...
			while (reader.Read())
			{
				...
				if (criteria.OnlyPlayed || criteria.OnlyUnplayed || criteria.MinPlayerAccuracy.HasValue ||
					criteria.MaxPlayerAccuracy.HasValue)
				{
					map.PlayerStats = GetMapPlayerStats(map.BeatmapPath, connection);

					if (criteria.OnlyPlayed && map.PlayCount == 0)
						continue;
					if (criteria.OnlyUnplayed && map.PlayCount > 0)
						continue;
					if (criteria.MinPlayerAccuracy.HasValue &&
						(map.BestPlayerAccuracy ?? 0) < criteria.MinPlayerAccuracy.Value)
						continue;
					if (criteria.MaxPlayerAccuracy.HasValue &&
						(map.BestPlayerAccuracy ?? 100) > criteria.MaxPlayerAccuracy.Value)
						continue;
				}
			...
			foreach (var map in maps) map.MsdScores = GetMapMsdScores(map.BeatmapPath, connection);
		}
		catch (Exception ex)
		{
	...
	public async Task ScanOsuSongsFolderAsync(string songsPath, CancellationToken cancellationToken = default)
	{
		...
		try
		{
			...
			Logger.Info(
				$"[MapsDB] Using {maxParallelism} parallel workers, {_mapInfoCache?.Count ?? 0} cached entries");
			...
			var flushTask = Task.Run(async () =>
			{
				try
				{
					while (!_indexingCts.Token.IsCancellationRequested)
					{
						...
						if (_pendingWrites.Count >= _batchWriteSize) FlushPendingWrites();
					}
				}
				catch (OperationCanceledException)
				{
				...
				lock (progressLock)
				{
					...
					if (processed % 100 == 0 || processed == total)
						IndexingProgressChanged?.Invoke(this, new IndexingProgressEventArgs
						{
							...
							Status =
								$"Indexing: {processed}/{total} ({processed * 100 / total}%) - {skipped} unchanged"
						});
				}
			});
	...
	private async Task<IndexResult> IndexBeatmapOptimizedAsync(string beatmapPath, CancellationToken cancellationToken)
	{
		...
		try
		{
			...
			if (!NeedsReindexing(beatmapPath, out var fileHash)) return IndexResult.Skipped;
			...
			if (isSupportedMania && _msdAnalyzer != null)
				try
				{
					...
					{
						var isSessionCopy = osuFile.Tags.Contains(BeatmapIndexer.SessionTag,
							StringComparison.OrdinalIgnoreCase);
						if (!isSessionCopy) _ = _beatmapApiService.UploadBeatmapAsync(beatmapPath, cancellationToken);
					}
				}
				catch (Exception ex)
				{
	...
	{
		return File.OpenRead(filePath).Md5();
	}
	...
	private void LoadMapInfoCache()
	{
		...
		try
		{
			...
			using var cmd = new SqliteCommand("SELECT BeatmapPath, FileHash, FileSize, LastModified FROM Maps",
				connection);
			using var reader = cmd.ExecuteReader();
	...
	public Dictionary<string, string> GetBeatmapPathsByHash()
	{
		...
		try
		{
			...
			while (reader.Read())
			{
				...
				if (!result.ContainsKey(hash)) result[hash] = path;
			}
	...
	public string? GetBeatmapPathByHash(string hash)
	{
		...
		try
		{
			...
			using var cmd =
				new SqliteCommand("SELECT BeatmapPath FROM Maps WHERE FileHash = @hash COLLATE NOCASE LIMIT 1",
					connection);
	...
	private void FlushPendingWrites()
	{
		...
		while (_pendingWrites.TryDequeue(out var map)) maps.Add(map);
		...
		lock (_batchWriteLock)
		{
			try
			{
				...
				foreach (var map in maps) SaveMapInternal(map, connection, transaction);
	...
	private void SaveMapInternal(IndexedMap map, SqliteConnection connection, SqliteTransaction transaction)
	{
		...
		using (var cmd = new SqliteCommand("DELETE FROM MapMsdScores WHERE BeatmapPath = @Path", connection,
				   transaction))
		{
	...
	private static IndexedMap ReadMap(SqliteDataReader reader)
	{
	...
	private static List<MapMsdScore> GetMapMsdScores(string beatmapPath, SqliteConnection connection)
	{
	...
	private static List<MapPlayerStat> GetMapPlayerStats(string beatmapPath, SqliteConnection connection)
	{
