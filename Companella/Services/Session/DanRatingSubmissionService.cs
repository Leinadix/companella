...
public class DanRatingSubmissionService : IDisposable
{
	...
	public async Task<bool> SubmitRatingAsync(string beatmapPath, string username, string danLabel, double accuracy)
	{
		...
		try
		{
			...
			// Add accuracy
			content.Add(new StringContent(accuracy.ToString("F2", System.Globalization.CultureInfo.InvariantCulture)),
				"accuracy");

			Logger.Info(
				$"[DanRating] Submitting rating: {danLabel} for {Path.GetFileName(beatmapPath)} (user: {username}, acc: {accuracy:F2}%)");
