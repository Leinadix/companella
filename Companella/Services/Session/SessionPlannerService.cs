...
public class SessionPlannerService
{
	...
	/// </summary>
	private const int _defaultMapDurationSeconds = 120;

	/// <summary>
	/// MSD tolerance when searching for maps.
	/// </summary>
	private const double _msdTolerance = 0.5;
	...
	private async Task<List<SessionPlanItem>> GenerateItemsFromCurveAsync(MsdCurveConfig curveConfig)
	{
		...
		for (var segment = 0; segment < segmentCount && currentDuration < totalDurationSeconds; segment++)
		{
			...
			ReportProgress(
				$"Finding maps for segment {segment + 1}/{segmentCount} (MSD: {targetMsd:F1}{skillsetInfo})...",
				progressPercent);
			...
			foreach (var map in segmentMaps)
			{
				...
				try
				{
					...
					var item = new SessionPlanItem
					{
						...
						EstimatedDurationSeconds = _defaultMapDurationSeconds
					};

					items.Add(item);
					usedPaths.Add(map.BeatmapPath);
					currentDuration += _defaultMapDurationSeconds;
				}
				catch (Exception ex)
				{
	...
	/// </summary>
	private static SessionPhase DeterminePhaseFromCurve(MsdCurveConfig curveConfig, double timePercent)
	{
		...
		SessionPlanConfig config)
	{
		...
		for (var segment = 0; segment < segmentCount && currentDuration < targetDurationSeconds; segment++)
		{
			double targetMsd;
		...
		else if (isIncreasing) items = items.OrderBy(i => i.ActualMsd).ToList();
	...
	/// </summary>
	private static void ReindexPlanItems(SessionPlan plan)
	{
		var index = 1;
		foreach (var item in plan.Items) item.Index = index++;
	}

	/// <summary>
	/// Creates indexed copies of all beatmaps in the plan.
	/// </summary>
	private static async Task<bool> CreateIndexedCopiesAsync(SessionPlan plan)
	{
		return await Task.Run(() =>
		{
			...
			foreach (var item in plan.Items)
				try
				{
					var indexedPath = BeatmapIndexer.CreateIndexedCopy(item.OriginalPath, item.Index);
			...
			foreach (var failed in failedItems) plan.Items.Remove(failed);
