...
public class TrayIconService : IDisposable
{
	...
	public void Initialize()
	{
		...
		if (!_initialized.WaitOne(5000)) Logger.Info("[TrayIcon] Timeout waiting for tray icon initialization");
	}
		...
		if (_appContext != null)
			try
			{
				if (_contextMenu != null && _contextMenu.InvokeRequired)
					_contextMenu.BeginInvoke(new Action(() => { _appContext?.ExitThread(); }));
				else
