...
public class HitErrorReaderService
{
	...
	public TimingAnalysisResult? ReadHitErrorsFromMemory(string beatmapPath, float rate = 1.0f)
	{
		...
		try
		{
			lock (MemoryReaderLock)
			{
				...
				if (!readResult)
				{
					...
					if (_memoryReader.TryRead(generalData))
						Logger.Info(
							$"[HitErrorReader] GeneralData read OK - Status: {generalData.RawStatus}, AudioTime: {generalData.AudioTime}");
					else
			...
			var estimatedTimePerHit = 1000.0; // Rough estimate, will be refined

			for (var i = 0; i < hitErrors.Count; i++)
			{
			...
			Logger.Info(
				$"[HitErrorReader] Analysis complete: UR={result.UnstableRate:F2}, Mean={result.MeanDeviation:F2}ms");
	...
	/// </summary>
	public TimingAnalysisResult? ReadHitErrorsWithBeatmap(string beatmapPath, OsuFileParser fileParser,
		float rate = 1.0f)
	{
		...
		lock (MemoryReaderLock)
		{
			try
			{
				...
				{
					Logger.Info(
						$"[HitErrorReader] GeneralData OK - Status: {generalData.RawStatus}, Mods: {generalData.Mods}");
				}
				else
				{
		...
		try
		{
			...
			var od = osuFile.OverallDifficulty;
			...
			if (osuFile.RawSections.TryGetValue("HitObjects", out var hitObjectLines))
				foreach (var line in hitObjectLines)
				{
					...
					if (hitObject != null)
					{
						...
						if (hitObject.IsHold) hitObjectTimes.Add(hitObject.EndTime);
					}
				}
			...
			Logger.Info(
				$"[HitErrorReader] Beatmap has {hitObjectTimes.Count} hit points, got {hitErrors.Count} errors");
			...
			Logger.Info(
				$"[HitErrorReader] Analysis complete: {result.Deviations.Count} deviations, UR={result.UnstableRate:F2}, Mean={result.MeanDeviation:F2}ms");
