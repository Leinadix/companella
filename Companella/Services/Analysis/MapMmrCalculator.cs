...
public class MapMmrCalculator
{
	...
	/// </summary>
	private const double _baseMsdWeight = 0.6;

	/// <summary>
	/// Weight for performance adjustment in MMR calculation (0-1).
	/// </summary>
	private const double _performanceWeight = 0.4;
	...
	public static MapMmrResult CalculateMapMmr(IndexedMap map, SkillsTrendResult playerTrends,
		string? targetSkillset = null)
	{
		...
		result.Mmr = baseMsd * _baseMsdWeight + adjustedMsd * _performanceWeight;
	...
	public static List<MapMmrResult> RankMaps(List<IndexedMap> maps, SkillsTrendResult trends, string? targetSkillset = null)
	{
	...
	/// </summary>
	private static double GetPlayerSkillForSkillset(SkillsTrendResult trends, string? skillset)
	{
		if (string.IsNullOrEmpty(skillset)) return trends.OverallSkillLevel;

		var key = skillset.ToLowerInvariant();
		if (trends.CurrentSkillLevels.TryGetValue(key, out var level)) return level;
	...
	/// </summary>
	private static double CalculatePerformanceAdjustment(IndexedMap map, double playerSkill)
	{
	...
	/// </summary>
	private static double CalculateConfidence(IndexedMap map, SkillsTrendResult trends)
	{
		double confidence = 0;

		// More MSD scores = more confidence
		if (map.MsdScores.Count > 0) confidence += 0.4;

		// More player history = more confidence
		if (map.PlayCount > 0) confidence += Math.Min(0.3, map.PlayCount * 0.1);

		// More trend data = more confidence
		if (trends.TotalPlays >= 10)
			confidence += 0.3;
		else if (trends.TotalPlays >= 5) confidence += 0.15;
	...
	public static float? GetSuggestedRate(IndexedMap map, float targetMsd)
	{
		if (map.MsdScores.Count == 0) return null;
