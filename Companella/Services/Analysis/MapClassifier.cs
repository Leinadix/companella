...
public class MapClassifier
{
	...
	/// </summary>
	public static MapComparisonData CreateComparisonData(SingleRateMsdResult? msdResult,
		PatternAnalysisResult? patterns, OsuFile? osuFile)
	{
	...
	/// </summary>
	public static MapComparisonData CreateComparisonData(MsdResult? msdResult, PatternAnalysisResult? patterns,
		OsuFile? osuFile)
	{
		return new MapComparisonData
		{
	...
	/// </summary>
	public static PatternMetrics GetPatternMetrics(PatternAnalysisResult? patterns)
	{
		if (patterns == null || !patterns.Success) return new PatternMetrics();
		...
		if (allBpms.Count > 0) metrics.AverageBpm = allBpms.Average();
	...
	/// </summary>
	public static MsdMetrics GetMsdMetrics(SkillsetScores? scores)
	{
		if (scores == null) return new MsdMetrics();
	...
	/// </summary>
	public static BpmMetrics GetBpmMetrics(PatternAnalysisResult? patterns)
	{
		if (patterns == null || !patterns.Success) return new BpmMetrics();

		var allPatterns = patterns.Patterns.Values.SelectMany(p => p).ToList();

		if (allPatterns.Count == 0) return new BpmMetrics();
	...
	/// </summary>
	private static Dictionary<string, int> GetBpmDistribution(List<double> bpms)
	{
		...
		foreach (var bpm in bpms)
			if (bpm < 100)
				...
				distribution["100-140"]++;
			else if (bpm < 180)
				distribution["140-180"]++;
			else if (bpm < 220)
				distribution["180-220"]++;
			else if (bpm < 260)
				distribution["220-260"]++;
			else
				distribution["260+"]++;
	...
	/// </summary>
	public static MapClassification ClassifyMap(MapComparisonData data)
	{
	...
	/// </summary>
	private static int CalculateLevelIndex(float overallMsd, double peakBpm, int patternVariety)
	{
		...
		if (overallMsd < 5)
			level = 1;
		else if (overallMsd < 8)
			level = 2;
		else if (overallMsd < 11)
			level = 3;
		else if (overallMsd < 14)
			level = 4;
		else if (overallMsd < 17)
			level = 5;
		else if (overallMsd < 20)
			level = 6;
		else if (overallMsd < 23)
			level = 7;
		else if (overallMsd < 26)
			level = 8;
		else if (overallMsd < 29)
			level = 9;
		else if (overallMsd < 32)
			level = 10;
		else if (overallMsd < 34)
			level = 11; // alpha
		else if (overallMsd < 36)
			level = 12; // beta
		else if (overallMsd < 38)
			level = 13; // gamma
		else if (overallMsd < 40)
			level = 14; // delta
		else if (overallMsd < 42)
			level = 15; // epsilon
		else if (overallMsd < 44)
			level = 16; // zeta
		else if (overallMsd < 46)
			level = 17; // eta
		else if (overallMsd < 48)
			level = 18; // theta
		else if (overallMsd < 50)
			level = 19; // iota
		else
			level = 20; // kappa (max)

		// Adjust for BPM (placeholder - would need more sophisticated logic)
		if (peakBpm > 220 && level < 20) level = Math.Min(level + 1, 20);
