...
public class InterludeDifficultyService
{
	...
	private const float _curvePower = 0.6f;
	private const float _curveScale = 0.4056f;
	private const float _mostImportantNotes = 2500.0f;

	// Constants from YAVSRG's Notes.fs
	private const float _jackCurveCutoff = 230.0f;
	private const float _streamCurveCutoff = 10.0f;
	private const float _streamCurveCutoff2 = 10.0f;
	private const float _ohtNerf = 3.0f;
	private const float _streamScale = 6.0f;
	private const float _streamPow = 0.5f;

	// Constants from YAVSRG's Strain.fs
	private const float _strainScale = 0.01626f;
	private const float _strainTimeCap = 200.0f; // ms/rate
	...
	private static float WeightedOverallDifficulty(IEnumerable<float> data)
	{
		...
		for (var i = 0; i < dataArray.Length; i++)
		{
			...
			var position = (i + _mostImportantNotes - length) / _mostImportantNotes;
		...
		var result = (float)Math.Pow(weightedAverage, _curvePower) * _curveScale;
	...
	{
		return Math.Min(15000.0f / deltaMs, _jackCurveCutoff);
	}
	...
	private static float MsToStreamBpm(float deltaMs)
	{
		var result = 300.0f / (0.02f * deltaMs) -
					 300.0f / (float)Math.Pow(0.02f * deltaMs, _streamCurveCutoff) / _streamCurveCutoff2;
	...
	private static float CalculateNoteTotal(float j, float sl, float sr)
	{
		return (float)Math.Pow(
			Math.Pow(_streamScale * (float)Math.Pow(sl, _streamPow), _ohtNerf) +
			Math.Pow(_streamScale * (float)Math.Pow(sr, _streamPow), _ohtNerf) +
			Math.Pow(j, _ohtNerf),
			1.0f / _ohtNerf
		);
	}
	...
	private static float StrainFunc(float halfLifeMs, float currentValue, float input, float deltaMs)
	{
		...
		var decay = (float)Math.Exp(decayRate * Math.Min(_strainTimeCap, deltaMs));
		var timeCapDecay = deltaMs > _strainTimeCap
			? (float)Math.Exp(decayRate * (deltaMs - _strainTimeCap))
			: 1.0f;

		var a = currentValue * timeCapDecay;
		var b = input * input * _strainScale;
		return b - (b - a) * decay;
	}
		...
		foreach (var row in noteRows)
		{
			...
			for (var k = 0; k < keyCount; k++)
			{
				...
				if (hasNote)
				{
					...
					var sl = 0.0f; // stream left
					var sr = 0.0f; // stream right

					for (var handK = handLo; handK <= handHi; handK++)
						if (handK != k)
						{
