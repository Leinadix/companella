...
/// </summary>
public class DanModelService : IDisposable
{
	/// <summary>
	...
	private const string _modelFileName = "dan_model.onnx";
	private const int _featureCount = 10; // 8 MSD + 1 Interlude + 1 Sunny
	private const float _minDan = 1.0f;
	private const float _maxDan = 20.0f;
	...
	public DanModelService()
	{
		...
		_modelPath = Path.Combine(DataPaths.ApplicationFolder, _modelFileName);
	}
	...
	public float PredictRaw(MsdSkillsetValues msd, double interludeRating, double sunnyRating)
	{
		...
		try
		{
			...
			Logger.Info(
				$"[DanModel] Input: Overall={features[0]:F2}, Stream={features[1]:F2}, JS={features[2]:F2}, HS={features[3]:F2}, Stam={features[4]:F2}, Jack={features[5]:F2}, CJ={features[6]:F2}, Tech={features[7]:F2}, Interlude={features[8]:F2}, Sunny={features[9]:F2}");

			// Create input tensor with shape [1, 10]
			var inputTensor = new DenseTensor<float>(features, new[] { 1, _featureCount });
			...
			return Math.Clamp(rawValue, _minDan, _maxDan);
		}
		catch (Exception ex)
		{
	...
	public static (int DanIndex, string? Variant) ParsePrediction(float rawValue)
	{
		if (rawValue < _minDan)
			return (0, "--");
	...
	public DanClassificationResult? ClassifyMap(SkillsetScores? msdScores, double interludeRating, double sunnyRating)
	{
		...
		var danLevel = (int)Math.Round(rawValue);
		var offset = Math.Abs(rawValue - danLevel);
		var confidence = Math.Max(0, 1.0 - offset * 2); // 0 offset = 1.0, 0.5 offset = 0.0
	...
	public DanClassificationResult? ClassifyMap(MsdSkillsetValues msd, double interludeRating, double sunnyRating)
	{
		...
		var danLevel = (int)Math.Round(rawValue);
		var offset = Math.Abs(rawValue - danLevel);
		var confidence = Math.Max(0, 1.0 - offset * 2);
