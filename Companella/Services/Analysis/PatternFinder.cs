...
public class PatternFinder
{
	...
	private const double _maxPatternIntervalMs = 250.0; // Max interval between notes in a pattern (about 60 BPM 1/4)
	private const int _minTrillNotes = 4;
	private const int _minJackNotes = 3;
	private const int _minStreamNotes = 4;
	private const int _minRollNotes = 4;
	private const double _minijackMaxIntervalMs = 100.0;
	private const int _minChordstreamNotes = 6;
	private const int _minBracketNotes = 6;
	private const double _maxRollIntervalMs = 150.0; // Rolls are fast patterns
	...
	/// </summary>
	public static PatternAnalysisResult FindAllPatterns(List<HitObject> hitObjects)
	{
	...
	/// </summary>
	private static Dictionary<double, List<HitObject>> GroupNotesByTime(List<HitObject> hitObjects,
		double toleranceMs = 2.0)
	{
	...
	/// </summary>
	private static void DetectChords(Dictionary<double, List<HitObject>> notesByTime, PatternAnalysisResult result)
	{
		...
		if (jumps.Count > 0)
			result.Patterns[PatternType.Jump] = jumps;
		if (hands.Count > 0)
			result.Patterns[PatternType.Hand] = hands;
		if (quads.Count > 0)
			result.Patterns[PatternType.Quad] = quads;
	}
	...
	/// </summary>
	private static void DetectJacks(List<HitObject> hitObjects, PatternAnalysisResult result)
	{
		...
		foreach (var kvp in byColumn)
		{
			...
			for (int i = 1; i < notes.Count; i++)
			{
				...
				if (interval <= _maxPatternIntervalMs && interval > 0)
				{
					jackCount++;
				...
				else
				{
					...
					else if (jackCount == 2)
					{
						...
						if (mjInterval <= _minijackMaxIntervalMs)
						{
							var bpm = CalculateBpmFromDelta(mjInterval);
							minijacks.Add(CreatePatternWithBpm(PatternType.Minijack, notes[jackStart].Time,
								notes[jackStart + 1].Time, new List<int> { column }, 2, bpm));
						}
					}
		...
		if (jacks.Count > 0)
			result.Patterns[PatternType.Jack] = jacks;
		if (minijacks.Count > 0)
			result.Patterns[PatternType.Minijack] = minijacks;
	}
	...
	/// </summary>
	private static void DetectTrills(List<HitObject> hitObjects, PatternAnalysisResult result)
	{
		var trills = new List<Pattern>();

		if (hitObjects.Count < _minTrillNotes)
			return;
		...
		for (int i = 1; i < hitObjects.Count; i++)
		{
			...
			{
				// Too slow, end trill
				if (trillCount >= _minTrillNotes && isTrill)
				{
					var bpm = CalculateBpmFromTimes(trillTimes);
					trills.Add(CreatePatternWithBpm(PatternType.Trill, hitObjects[trillStart].Time,
						hitObjects[i - 1].Time, new List<int> { col1, col2 }, trillCount, bpm));
				}
		...
		{
			var bpm = CalculateBpmFromTimes(trillTimes);
			trills.Add(CreatePatternWithBpm(PatternType.Trill, hitObjects[trillStart].Time, hitObjects[^1].Time,
				new List<int> { col1, col2 }, trillCount, bpm));
		}

		if (trills.Count > 0)
			result.Patterns[PatternType.Trill] = trills;
	}
	...
	/// </summary>
	private static void DetectStreams(List<HitObject> hitObjects, Dictionary<double, List<HitObject>> notesByTime,
		PatternAnalysisResult result)
	{
		...
		if (singleNotes.Count < _minStreamNotes)
			return;
		...
		for (int i = 1; i < singleNotes.Count; i++)
		{
			...
			if (current.Column != lastColumn && interval <= _maxPatternIntervalMs && interval > 0)
			{
				streamCount++;
			...
			{
				// End of stream
				if (streamCount >= _minStreamNotes)
				{
					var bpm = CalculateBpmFromTimes(streamTimes);
					streams.Add(CreatePatternWithBpm(PatternType.Stream, singleNotes[streamStart].Time,
						singleNotes[i - 1].Time, columns.ToList(), streamCount, bpm));
				}
		...
		{
			var bpm = CalculateBpmFromTimes(streamTimes);
			streams.Add(CreatePatternWithBpm(PatternType.Stream, singleNotes[streamStart].Time, singleNotes[^1].Time,
				columns.ToList(), streamCount, bpm));
		}

		if (streams.Count > 0)
			result.Patterns[PatternType.Stream] = streams;
	}
	...
	/// </summary>
	private static void DetectRolls(List<HitObject> hitObjects, PatternAnalysisResult result)
	{
		var rolls = new List<Pattern>();

		if (hitObjects.Count < _minRollNotes)
			return;
		...
		for (int i = 1; i < hitObjects.Count; i++)
		{
			...
			if (interval <= _maxRollIntervalMs && interval > 0 && colDiff != 0)
			{
				...
				{
					// Direction changed
					if (rollCount >= _minRollNotes)
					{
						var bpm = CalculateBpmFromTimes(rollTimes);
						rolls.Add(CreatePatternWithBpm(PatternType.Roll, hitObjects[rollStart].Time,
							hitObjects[i - 1].Time, columns.ToList(), rollCount, bpm));
					}
			...
			else
			{
				...
				{
					var bpm = CalculateBpmFromTimes(rollTimes);
					rolls.Add(CreatePatternWithBpm(PatternType.Roll, hitObjects[rollStart].Time, hitObjects[i - 1].Time,
						columns.ToList(), rollCount, bpm));
				}
		...
		{
			var bpm = CalculateBpmFromTimes(rollTimes);
			rolls.Add(CreatePatternWithBpm(PatternType.Roll, hitObjects[rollStart].Time, hitObjects[^1].Time,
				columns.ToList(), rollCount, bpm));
		}

		if (rolls.Count > 0)
			result.Patterns[PatternType.Roll] = rolls;
	}
	...
	/// </summary>
	private static void DetectChordstreams(List<HitObject> hitObjects, Dictionary<double, List<HitObject>> notesByTime,
		PatternAnalysisResult result)
	{
		...
		if (sortedTimes.Count < _minChordstreamNotes)
			return;
		...
		for (int i = 0; i < sortedTimes.Count; i++)
		{
			...
			if (interval <= _maxPatternIntervalMs || i == 0)
			{
				noteCount += chordSize;
				eventTimes.Add(time);
			...
			else
			{
				...
				if (noteCount >= _minChordstreamNotes)
				{
					var bpm = CalculateBpmFromTimes(eventTimes);
					if (handCount > 0)
					{
						handstreams.Add(CreatePatternWithBpm(PatternType.Handstream, sortedTimes[csStart],
							sortedTimes[i - 1], columns.ToList(), noteCount, bpm));
					}
					else if (jumpCount > 0)
					{
						jumpstreams.Add(CreatePatternWithBpm(PatternType.Jumpstream, sortedTimes[csStart],
							sortedTimes[i - 1], columns.ToList(), noteCount, bpm));
					}
				}
		...
		if (noteCount >= _minChordstreamNotes)
		{
			...
			if (handCount > 0)
			{
				handstreams.Add(CreatePatternWithBpm(PatternType.Handstream, sortedTimes[csStart], sortedTimes[^1],
					columns.ToList(), noteCount, bpm));
			}
			else if (jumpCount > 0)
			{
				jumpstreams.Add(CreatePatternWithBpm(PatternType.Jumpstream, sortedTimes[csStart], sortedTimes[^1],
					columns.ToList(), noteCount, bpm));
			}
		}

		if (jumpstreams.Count > 0)
			result.Patterns[PatternType.Jumpstream] = jumpstreams;
		if (handstreams.Count > 0)
			result.Patterns[PatternType.Handstream] = handstreams;
	}
	...
	/// BPM is calculated from chord event deltas.
	/// </summary>
	private static void DetectChordjacks(Dictionary<double, List<HitObject>> notesByTime, PatternAnalysisResult result)
	{
		...
		for (int i = 1; i < chordTimes.Count; i++)
		{
			...
			if (interval <= _maxPatternIntervalMs && overlap > 0)
			{
				cjCount++;
			...
			else
			{
				...
				{
					var bpm = CalculateBpmFromTimes(eventTimes);
					chordjacks.Add(CreatePatternWithBpm(PatternType.Chordjack, chordTimes[cjStart].Key,
						chordTimes[i - 1].Key, columns.ToList(), noteCount, bpm));
				}
		...
		{
			var bpm = CalculateBpmFromTimes(eventTimes);
			chordjacks.Add(CreatePatternWithBpm(PatternType.Chordjack, chordTimes[cjStart].Key, chordTimes[^1].Key,
				columns.ToList(), noteCount, bpm));
		}

		if (chordjacks.Count > 0)
			result.Patterns[PatternType.Chordjack] = chordjacks;
	}
	...
	/// </summary>
	private static void DetectJumptrills(Dictionary<double, List<HitObject>> notesByTime, PatternAnalysisResult result)
	{
		...
		for (int i = 0; i < jumpTimes.Count; i++)
		{
			...
			if (interval > _maxPatternIntervalMs)
			{
				// End of jumptrill
				if (jtCount >= 4)
				{
					var bpm = CalculateBpmFromTimes(eventTimes);
					jumptrills.Add(CreatePatternWithBpm(PatternType.Jumptrill, jumpTimes[jtStart].Key,
						jumpTimes[i - 1].Key, allColumns.ToList(), noteCount, bpm));
				}
		...
		{
			var bpm = CalculateBpmFromTimes(eventTimes);
			jumptrills.Add(CreatePatternWithBpm(PatternType.Jumptrill, jumpTimes[jtStart].Key, jumpTimes[^1].Key,
				allColumns.ToList(), noteCount, bpm));
		}

		if (jumptrills.Count > 0)
			result.Patterns[PatternType.Jumptrill] = jumptrills;
	}
	...
	/// </summary>
	private static void DetectBrackets(List<HitObject> hitObjects, PatternAnalysisResult result)
	{
		var brackets = new List<Pattern>();

		if (hitObjects.Count < _minBracketNotes)
			return;
		...
		for (int i = 1; i < hitObjects.Count; i++)
		{
			...
			if (interval <= _maxRollIntervalMs && interval > 0 && colDiff <= 2 && colDiff >= 1)
			{
				bracketCount++;
				columns.Add(current.Column);
			...
			{
				if (bracketCount >= _minBracketNotes && columns.Count >= 3)
				{
					var bpm = CalculateBpmFromTimes(bracketTimes);
					brackets.Add(CreatePatternWithBpm(PatternType.Bracket, hitObjects[bracketStart].Time,
						hitObjects[i - 1].Time, columns.ToList(), bracketCount, bpm));
				}
		...
		{
			var bpm = CalculateBpmFromTimes(bracketTimes);
			brackets.Add(CreatePatternWithBpm(PatternType.Bracket, hitObjects[bracketStart].Time, hitObjects[^1].Time,
				columns.ToList(), bracketCount, bpm));
		}

		if (brackets.Count > 0)
			result.Patterns[PatternType.Bracket] = brackets;
	}

	/// <summary>
	/// Creates a Pattern object with the specified BPM.
	/// </summary>
	private static Pattern CreatePatternWithBpm(PatternType type, double startTime, double endTime, List<int> columns,
		int noteCount, double bpm)
	{
		return new Pattern
		{
			Type = type,
