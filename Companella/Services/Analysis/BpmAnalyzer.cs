...
public class BpmAnalysisOptions
{
	...
	/// </summary>
	public bool UsePercussion { get; set; }
...
public class BpmAnalyzer
{
	...
	public async Task<BpmResult> AnalyzeAsync(string audioPath, BpmAnalysisOptions? options = null)
	{
		...
		var arguments =
			$"\"{audioPath}\" -j --persist 3 --min-gap-ms 50 --offset-threshold-ms 0 --drift-slope-ms-per-beat 0.7 --bpm-min-change 0.2";
		...
		Logger.Info(
			$"[BPM] Process completed: {completed}, Exit code: {(completed ? process.ExitCode.ToString(CultureInfo.InvariantCulture) : "N/A")}");
		...
		catch (JsonException ex)
		{
			...
			throw new InvalidOperationException(
				$"Failed to parse BPM analysis JSON: {ex.Message}\nOutput: {jsonOutput}");
		}
	}
