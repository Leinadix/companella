...
public class MarathonCreatorService
{
	...
	private class PreviewCache
	{
		...
		public void Dispose()
		{
			...
			foreach (var bg in CachedBackgrounds.Values) bg.Dispose();
	...
	private static SixLabors.Fonts.FontFamily GetEmbeddedFontFamily()
	{
		lock (_fontLock)
		{
			...
			if (stream != null)
			{
				...
				_notoFontFamily = _fontCollection.Add(stream, CultureInfo.InvariantCulture);
			...
			try
			{
				_notoFontFamily = SixLabors.Fonts.SystemFonts.Get("Noto Sans SC", CultureInfo.InvariantCulture);
				return _notoFontFamily.Value;
			}
			catch
			{
				try
				{
					_notoFontFamily = SixLabors.Fonts.SystemFonts.Get("Microsoft YaHei", CultureInfo.InvariantCulture);
					return _notoFontFamily.Value;
				}
				catch
				{
					...
					{
						_notoFontFamily =
							SixLabors.Fonts.SystemFonts.Get("Segoe UI Symbol", CultureInfo.InvariantCulture);
						return _notoFontFamily.Value;
					}
					catch
					{
						_notoFontFamily = SixLabors.Fonts.SystemFonts.Get("Arial", CultureInfo.InvariantCulture);
						return _notoFontFamily.Value;
					}
				}
			}
	...
	/// </summary>
	private static void TrackRateChangedFiles(string osuPath, OsuFile osuFile, List<string> tempRateChangedFiles)
	{
		// Track the .osu file
	...
	/// </summary>
	private static void CleanupTempRateChangedFiles(List<string> tempRateChangedFiles)
	{
		...
		CancellationToken cancellationToken = default)
	{
		...
		try
		{
			...
			try
			{
				...
				var fadeInfo = await CalculateFadeInfoAsync(entries, tempDir, tempRateChangedFiles, progressCallback,
					cancellationToken);
				...
				for (var i = 0; i < entries.Count; i++)
				{
					...
					if (entry.IsPause)
					{
						...
						progressCallback?.Invoke(
							$"Processing pause {i + 1}/{entries.Count}: {remainingPauseDuration:F0}ms remaining");
						...
						await GenerateSilentAudioAsync(silentAudioPath, remainingPauseDuration / 1000.0,
							cancellationToken);
						...
						if (actualPauseDuration > 0)
						{
							...
							Logger.Info(
								$"[Marathon] Pause: using actual duration {actualPauseDuration:F0}ms (requested {remainingPauseDuration:F0}ms)");
						}

						segment.ShiftedTimingPointLines.Add(
							$"{cumulativeOffset.ToString("0.###", CultureInfo.InvariantCulture)},500,4,2,0,100,1,0");
		...
		CancellationToken cancellationToken)
	{
		...
		var mapAudioInfo =
			new Dictionary<int, (double FirstNoteTime, double LastNoteTime, double AudioDuration, OsuFile WorkingOsu
				)>();
		...
		for (var i = 0; i < entries.Count; i++)
		{
			...
			if (entry.IsPause)
			{
				fadeInfo.Add(new EntryFadeInfo
				{ Index = i, IsPause = true, RemainingPauseDurationMs = entry.PauseDurationSeconds * 1000 });
				continue;
			}
			...
			var firstNoteTime = hitObjects.Count > 0 ? hitObjects.Min(h => h.Time) : 0;
			var lastNoteTime = hitObjects.Count > 0 ? hitObjects.Max(h => h.EndTime) : 0;
		...
		for (var i = 0; i < entries.Count; i++)
		{
			...
			if (entry.IsPause)
			{
				...
				if (i > 0 && !entries[i - 1].IsPause && mapAudioInfo.ContainsKey(i - 1))
				{
					...
					Logger.Info(
						$"[Marathon] Map {i - 1} outro: {availableOutro:F0}ms available, extension={outroExtension:F0}ms, fade={outroExtension:F0}ms");
				}
				...
				if (i < entries.Count - 1 && !entries[i + 1].IsPause && mapAudioInfo.ContainsKey(i + 1))
				{
					...
					Logger.Info(
						$"[Marathon] Map {i + 1} intro: {availableIntro:F0}ms available, extension={introExtension:F0}ms, fade={introExtension:F0}ms");
				}
				...
				Logger.Info(
					$"[Marathon] Pause {i}: {pauseDurationMs:F0}ms total, {consumedByPrevious:F0}ms consumed by prev, {consumedByNext:F0}ms consumed by next, {info.RemainingPauseDurationMs:F0}ms remaining");
			}
		}
		...
		CancellationToken cancellationToken)
	{
		...
		var workingOsuFile = entry.OsuFile!;
		...
		var firstNoteTime = hitObjects.Count > 0 ? hitObjects.Min(h => h.Time) : 0;
		var lastNoteEndTime = hitObjects.Count > 0 ? hitObjects.Max(h => h.EndTime) : 0;
		...
		var trimStartMs = firstNoteTime - introExtensionMs;
		var trimEndMs = lastNoteEndTime + outroExtensionMs;
		...
		var noteDuration = lastNoteEndTime - firstNoteTime;
		...
		progressCallback?.Invoke(
			$"  Trimming audio (ext: {introExtensionMs:F0}/{outroExtensionMs:F0}ms, fade: {introFadeDurationMs:F0}/{outroFadeDurationMs:F0}ms)...");
		...
		if (actualAudioDuration > 0)
		{
			...
			if (durationDiff > 100) // More than 100ms difference is suspicious
				Logger.Info(
					$"[Marathon] WARNING: Audio duration mismatch for {entry.Title}: calculated={calculatedDuration:F0}ms, actual={actualAudioDuration:F0}ms");

			segment.Duration = actualAudioDuration;
			Logger.Info(
				$"[Marathon] {entry.Title}: using actual audio duration {actualAudioDuration:F0}ms (calculated was {calculatedDuration:F0}ms)");
		}
		...
		CancellationToken cancellationToken)
	{
		...
		// -t sets duration
		var arguments =
			$"-y -f lavfi -i anullsrc=r=44100:cl=stereo -t {durationSeconds.ToString("0.###", CultureInfo.InvariantCulture)} -c:a libmp3lame -q:a 2 \"{outputPath}\"";
	...
	private static async Task<double> TryGetDurationWithFfprobeAsync(string audioPath,
		CancellationToken cancellationToken)
	{
		try
		{
	...
	private async Task<double> TryGetDurationWithFfmpegAsync(string audioPath, CancellationToken cancellationToken)
	{
		try
		{
			...
			if (match.Success)
			{
				...
				var centiseconds = int.Parse(match.Groups[4].Value.PadRight(2, '0')[..2], CultureInfo.InvariantCulture);
		...
		CancellationToken cancellationToken)
	{
		...
		if (fadeOutDurationMs > 0)
		{
			...
			filterParts.Add(
				$"afade=t=out:st={fadeOutStart.ToString("0.###", CultureInfo.InvariantCulture)}:d={fadeOutSec.ToString("0.###", CultureInfo.InvariantCulture)}");
		}
		...
		if (filterParts.Count > 0)
		{
			...
			arguments =
				$"-y -ss {startSecStr} -t {durationSec} -i \"{inputPath}\" -af \"{filterChain}\" -c:a libmp3lame -q:a 2 \"{outputPath}\"";
		}
		else
		{
			// No fades, just trim
			arguments =
				$"-y -ss {startSecStr} -t {durationSec} -i \"{inputPath}\" -c:a libmp3lame -q:a 2 \"{outputPath}\"";
		}
		...
		CancellationToken cancellationToken)
	{
		...
		foreach (var path in inputPaths)
		{
			...
			concatContent.AppendLine(CultureInfo.InvariantCulture, $"file '{escapedPath}'");
		}
	...
	private static void ShiftAndCollectOsuData(
		OsuFile osuFile,
		...
		ProcessedMapSegment segment)
	{
		...
		if (osuFile.RawSections.TryGetValue("Editor", out var editorLines))
			foreach (var line in editorLines)
			{
				...
				if (trimmedLine.StartsWith("Bookmarks:", StringComparison.OrdinalIgnoreCase))
				{
					var colonIndex = trimmedLine.IndexOf(':');
					if (colonIndex >= 0)
					{
						var bookmarksStr = trimmedLine[(colonIndex + 1)..].Trim();
	...
	/// <param name="firstNoteInMarathon">Where the first note of this segment is in the marathon.</param>
	/// <param name="segment">The segment to collect data into.</param>
	private static void ProcessTimingPoints(
		OsuFile osuFile,
		double trimStartTime,
		double segmentStart,
		double firstNoteInMarathon,
		ProcessedMapSegment segment)
	{
		...
		if (timingPoints.Count == 0)
		{
			...
			segment.ShiftedTimingPointLines.Add(
				$"{firstNoteInMarathon.ToString("0.###", CultureInfo.InvariantCulture)},500,4,2,0,100,1,0");
			return;
		}
		...
		if (lastBpmBeforeTrim == null) lastBpmBeforeTrim = timingPoints.FirstOrDefault(tp => tp.Uninherited);
		...
		if (lastBpmBeforeTrim != null)
		{
			...
			Logger.Info(
				$"[Marathon] Added BPM point at first note: {firstNoteInMarathon:F0}ms, BPM={60000.0 / lastBpmBeforeTrim.BeatLength:F1}");
		}
		...
		foreach (var tp in timingPoints)
		{
			...
			if (tp.Time <= trimStartTime) continue;
	...
	private static string? ShiftHitObjectLine(string line, double trimStartTime, double segmentStart)
	{
		...
		try
		{
			...
			if (parts.Length >= 6 && parts[5].Contains(':'))
			{
				...
				if (long.TryParse(endParts[0], out var endTime))
				{
					...
					if (newEndTime < segmentStart)
						newEndTime = segmentStart;
	...
	/// <param name="trimStartTime">The start time of the trimmed audio in the original file.</param>
	/// <param name="segmentStart">Where this segment starts in the marathon.</param>
	private static string ShiftEventLine(string line, double trimStartTime, double segmentStart)
	{
		// Background images - keep as-is (will need to handle separately)
		if (line.StartsWith("0,0,", StringComparison.OrdinalIgnoreCase))
			return line;
		...
		if (line.StartsWith("2,", StringComparison.OrdinalIgnoreCase) ||
			line.StartsWith("Break,", StringComparison.OrdinalIgnoreCase))
		{
			var parts = line.Split(',');
			if (parts.Length >= 3)
				try
				{
					if (int.TryParse(parts[1], out var startTime))
					{
						...
						parts[1] = ((int)newStart).ToString(CultureInfo.InvariantCulture);
					}
					...
					if (int.TryParse(parts[2], out var endTime))
					{
						...
						if (newEnd < segmentStart)
							newEnd = segmentStart;
						parts[2] = ((int)newEnd).ToString(CultureInfo.InvariantCulture);
					}
	...
	/// </summary>
	private static string CreateMergedOsuFile(
		...
		string audioFilename)
	{
		...
		lines.Add(
			$"TitleUnicode:{(string.IsNullOrEmpty(metadata.TitleUnicode) ? metadata.Title : metadata.TitleUnicode)}");
		lines.Add($"Artist:{metadata.Artist}");
		lines.Add(
			$"ArtistUnicode:{(string.IsNullOrEmpty(metadata.ArtistUnicode) ? metadata.Artist : metadata.ArtistUnicode)}");
		...
		foreach (var segment in segments)
			foreach (var eventLine in segment.ShiftedEventLines)
				if (eventLine.StartsWith("2,", StringComparison.OrdinalIgnoreCase) ||
					eventLine.StartsWith("Break,", StringComparison.OrdinalIgnoreCase))
					lines.Add(eventLine);
		...
		// Generate filename
		var osuFileName =
			SanitizeFileName($"{metadata.Artist} - {metadata.Title} ({metadata.Creator}) [{metadata.Version}].osu");
	...
	/// </summary>
	private static void NormalizeSV(string osuPath)
	{
		...
		try
		{
			var osuFile = OsuFileParser.Parse(osuPath);
			...
			OsuFileWriter.Write(osuFile, normalizedTimingPoints);
		...
		CancellationToken cancellationToken = default)
	{
		...
		if (mapEntries.Count == 0) return null;

		try
		{
			// Calculate scale factors
			var scaleX = previewWidth / (float)_bgWidth;
			var centerX = previewWidth / 2f;
			var centerY = previewHeight / 2f;
			var innerRadius = _innerRadius * scaleX;
			var outerRadius = _outerRadius * scaleX;
			var circleRadius = _centerCircleRadius * scaleX;
			var borderThickness = Math.Max(1f, _borderThickness * scaleX);

			// Check if we can use cached data for pan/zoom only update
			var useCache = panZoomOnly && _previewCache != null &&
						   _previewCache.IsValid(previewWidth, previewHeight, mapEntries.Count, centerText,
							   glitchIntensity);
			...
			else
			{
				...
				{
					var shardStartAngle = startAngle + i * anglePerShard;
					var shardEndAngle = shardStartAngle + anglePerShard;

					var shardPath = BuildShardPathScaled(shardStartAngle, shardEndAngle, centerX, centerY, innerRadius,
						outerRadius);
					shardPaths.Add(shardPath);

					var shardCenter = CalculateShardCenterScaled(shardStartAngle, shardEndAngle, centerX, centerY,
				...
				DrawShardBordersScaled(overlayLayer, shardPaths, centerX, centerY, innerRadius, outerRadius,
					borderThickness);
			...
			for (var i = 0; i < mapEntries.Count; i++)
			{
				...
				else
				{
					...
					shardImage =
						await LoadMapBackgroundForPreviewAsync(entry, previewWidth, previewHeight, cancellationToken);
				...
				DrawShardOntoCanvasScaled(previewImage, shardImage, shardPaths[i], shardCenters[i], previewWidth,
					previewHeight, entry.BackgroundPanX, entry.BackgroundPanY);

				if (shouldDispose) shardImage.Dispose();
			}

			// Pass 3: Composite overlay (borders, center circle) on top
			if (overlayLayer != null) previewImage.Mutate(ctx => ctx.DrawImage(overlayLayer, 1f));

			// Pass 4: Apply glitch effects to final composited image
			if (glitchIntensity > 0)
			{
				var scale = previewWidth / (float)_bgWidth;
	...
	/// </summary>
	private static async Task<Image<Rgba32>> LoadMapBackgroundForPreviewAsync(MarathonEntry entry, int width,
	...
	/// </summary>
	private static IPath BuildShardPathScaled(float startAngleDeg, float endAngleDeg, float centerX, float centerY,
		float innerRadius, float outerRadius)
	{
		var arcAngleSpan = endAngleDeg - startAngleDeg;
		var arcSegments = Math.Max(8, (int)(arcAngleSpan / 5));
		...
		for (var i = 1; i <= arcSegments; i++)
		{
			...
			var angle = startAngleDeg + arcAngleSpan * t;
		...
		var endRad = endAngleDeg * MathF.PI / 180f;
	...
	/// </summary>
	private static SixLabors.ImageSharp.PointF CalculateShardCenterScaled(float startAngleDeg, float endAngleDeg,
		float centerX, float centerY, float innerRadius, float outerRadius)
	{
		var midAngleDeg = (startAngleDeg + endAngleDeg) / 2f;
		var midRadius = (innerRadius + outerRadius) / 2f;
		var midRad = midAngleDeg * MathF.PI / 180f;
	...
	/// </summary>
	private static void DrawShardOntoCanvasScaled(Image<Rgba32> canvas, Image<Rgba32> shardImage, IPath shardPath,
		SixLabors.ImageSharp.PointF shardCenter, int width, int height, float panX, float panY)
	{
		// Center the (possibly zoomed) image on the shard center
		var offsetX = shardCenter.X - shardImage.Width / 2f;
		var offsetY = shardCenter.Y - shardImage.Height / 2f;

		// Apply pan offset - pan moves the entire image rectangle
		// Pan values are in normalized units, scale to base dimensions
		var panOffsetX = panX * width * 0.5f;
		var panOffsetY = panY * height * 0.5f;
		offsetX += panOffsetX;
		offsetY += panOffsetY;
		...
		canvas.Mutate(ctx =>
		{
			...
			ctx.Clip(shardPath,
				c => { c.DrawImage(shardLayer, new SixLabors.ImageSharp.Point((int)offsetX, (int)offsetY), 1f); });
		});
	}
	...
	/// </summary>
	private static void DrawShardBordersScaled(Image<Rgba32> canvas, List<IPath> shardPaths, float centerX,
		float centerY, float innerRadius, float outerRadius, float borderThickness)
	{
		var pen = SixLabors.ImageSharp.Drawing.Processing.Pens.Solid(SixLabors.ImageSharp.Color.Black, borderThickness);

		canvas.Mutate(ctx =>
		{
			foreach (var path in shardPaths) ctx.Draw(pen, path);
	...
	/// </summary>
	private static void DrawCenterCircleScaled(Image<Rgba32> canvas, string centerText, float centerX, float centerY,
		float circleRadius, float borderThickness)
	{
		...
		canvas.Mutate(ctx =>
		{
			...
			var pen = SixLabors.ImageSharp.Drawing.Processing.Pens.Solid(SixLabors.ImageSharp.Color.White,
				Math.Max(1f, borderThickness * 0.75f));
		...
		if (!string.IsNullOrWhiteSpace(centerText))
		{
			...
			var padding = 4f;
			var availableRadius = circleRadius - borderThickness / 2f - padding;
			var availableDiameter = availableRadius * 2f;
			...
			do
			{
				...
				var maxDimension = Math.Max(textBounds.Width, textBounds.Height);
			...
			var textCenterX = actualBounds.X + actualBounds.Width / 2f;
			var textCenterY = actualBounds.Y + actualBounds.Height / 2f;
			var originX = centerX - textCenterX;
			var originY = centerY - textCenterY;

			var textOptions = new RichTextOptions(font) { Origin = new System.Numerics.Vector2(originX, originY) };

			canvas.Mutate(ctx => { ctx.DrawText(textOptions, text, SixLabors.ImageSharp.Color.White); });
		}
	}
	...
	private const int _bgWidth = 1920;
	private const int _bgHeight = 1080;
	private const float _bgCenterX = _bgWidth / 2f;
	private const float _bgCenterY = _bgHeight / 2f;
	private const float _innerRadius = 80f; // Small circle cut-off in center
	private const float _outerRadius = 1100f; // Large enough to cover corners

	// Border and center circle settings
	private const float _borderThickness = 4f;
	private const float _centerCircleRadius = 70f;
	...
	/// </summary>
	private static async Task<HighlightInfo> GenerateMarathonBackgroundAsync(
		List<MarathonEntry> entries,
		MarathonMetadata metadata,
		string outputFolder,
		CancellationToken cancellationToken)
	{
		...
		try
		{
			...
			using var baseImage = new Image<Rgba32>(_bgWidth, _bgHeight);
			...
			for (var i = 0; i < mapEntries.Count; i++)
			{
				...
				Logger.Info(
					$"[Marathon] Processing shard {i + 1}/{mapEntries.Count}: {entry.Title} ({shardStartAngle:F1} to {shardEndAngle:F1} degrees)");
				...
				DrawShardOntoCanvas(baseImage, shardImage, shardPath, shardCenter, entry.BackgroundPanX,
					entry.BackgroundPanY);
			}
			...
			var glitchSeed = new Random().Next();
			...
			for (var i = 0; i < entryShardPaths.Count; i++)
			{
	...
	/// </summary>
	private static void AddHighlightToShard(Image<Rgba32> image, IPath shardPath)
	{
	...
	/// </summary>
	private static void InjectStoryboardIntoOsuFile(
		string osuFilePath,
		List<ProcessedMapSegment> segments,
	...
	/// </summary>
	private static void GenerateStructureDocument(
		List<ProcessedMapSegment> segments,
		MarathonMetadata metadata,
		string outputFolder)
	{
		...
		sb.AppendLine("================================================================================");
		sb.AppendLine();
		sb.AppendLine(CultureInfo.InvariantCulture, $"Title:    {metadata.Artist} - {metadata.Title}");
		sb.AppendLine(CultureInfo.InvariantCulture, $"Creator:  {metadata.Creator}");
		sb.AppendLine(CultureInfo.InvariantCulture, $"Version:  {metadata.Version}");
		sb.AppendLine();
		...
		sb.AppendLine(CultureInfo.InvariantCulture, $"Total Maps:       {mapCount}");
		sb.AppendLine(CultureInfo.InvariantCulture, $"Total Breaks:     {breakCount}");
		sb.AppendLine(CultureInfo.InvariantCulture, $"Total Duration:   {FormatDuration(totalDuration)}");
		sb.AppendLine(CultureInfo.InvariantCulture, $"Total Break Time: {FormatDuration(totalBreakTime)}");
		sb.AppendLine();
		sb.AppendLine("================================================================================");
		...
		sb.AppendLine(CultureInfo.InvariantCulture,
			$"{"#",-4} {"Type",-8} {"Start",-10} {"Duration",-10} {"Rate",-6} {"Title",-40} {"Difficulty",-20}");
		sb.AppendLine(new string('-', 120));
		...
		foreach (var segment in segments)
		{
			...
			{
				sb.AppendLine(CultureInfo.InvariantCulture,
					$"{trackNum,-4} {"BREAK",-8} {startTime,-10} {duration,-10} {"-",-6} {"--- Break ---",-40} {"-",-20}");
			}
			else
			{
				var entry = segment.Entry;
				var rate = entry.Rate.ToString(CultureInfo.InvariantCulture) + "x";
				var title = TruncateString(entry.Title, 38);
				var difficulty = TruncateString(entry.Version, 18);

				sb.AppendLine(CultureInfo.InvariantCulture,
					$"{trackNum,-4} {"MAP",-8} {startTime,-10} {duration,-10} {rate,-6} {title,-40} {difficulty,-20}");
			}
		...
		foreach (var segment in segments)
		{
			if (segment.Entry.IsPause)
			{
				sb.AppendLine(CultureInfo.InvariantCulture, $"[{trackNum}] BREAK");
				sb.AppendLine(CultureInfo.InvariantCulture,
					$"    Duration: {FormatDuration(segment.Duration)} ({segment.Duration:F0}ms)");
				sb.AppendLine();
			}
			else
			{
				...
				sb.AppendLine(CultureInfo.InvariantCulture, $"[{trackNum}] {entry.Title}");
				sb.AppendLine(CultureInfo.InvariantCulture, $"    Artist:     {osuFile?.Artist ?? "Unknown"}");
				sb.AppendLine(CultureInfo.InvariantCulture, $"    Creator:    {entry.Creator}");
				sb.AppendLine(CultureInfo.InvariantCulture, $"    Difficulty: {entry.Version}");
				sb.AppendLine(CultureInfo.InvariantCulture, $"    Rate:       {entry.Rate:0.00}x");
				sb.AppendLine(CultureInfo.InvariantCulture,
					$"    BPM:        {entry.Bpm:F1} (original: {entry.DominantBpm:F1})");
				sb.AppendLine(CultureInfo.InvariantCulture,
					$"    Duration:   {FormatDuration(segment.Duration)} ({segment.Duration:F0}ms)");
				sb.AppendLine(CultureInfo.InvariantCulture,
					$"    Start:      {FormatDuration(segment.OffsetInMarathon)}");
				...
				if (osuFile?.FilePath != null)
				{
					...
					sb.AppendLine(CultureInfo.InvariantCulture, $"    Path:       {relativePath}");
				}
				...
				if (entry.MsdValues != null)
				{
					...
					sb.AppendLine(CultureInfo.InvariantCulture,
						$"    MSD:        Overall={msd.Overall:F1}, Stream={msd.Stream:F1}, JS={msd.Jumpstream:F1}, HS={msd.Handstream:F1}");
					sb.AppendLine(CultureInfo.InvariantCulture,
						$"                Stamina={msd.Stamina:F1}, Jack={msd.Jackspeed:F1}, CJ={msd.Chordjack:F1}, Tech={msd.Technical:F1}");
				}
		...
		sb.AppendLine("================================================================================");
		sb.AppendLine(CultureInfo.InvariantCulture, $"Generated by Companella Marathon Creator");
	...
	private static string FormatDuration(double milliseconds)
	{
		...
		if (timeSpan.TotalHours >= 1) return $"{(int)timeSpan.TotalHours}:{timeSpan.Minutes:D2}:{timeSpan.Seconds:D2}";
	...
	private static string GetRelativePathFromSongs(string filePath)
	{
		...
		if (songsIndex >= 0) return normalizedPath[(songsIndex + 7)..]; // +7 to skip "\Songs\"
	...
	/// Applies zoom and pan transformations based on the entry's settings.
	/// </summary>
	private static async Task<Image<Rgba32>> LoadMapBackgroundAsync(MarathonEntry entry,
		CancellationToken cancellationToken)
	{
		if (entry.OsuFile?.BackgroundFilename != null)
		{
			...
			if (File.Exists(bgPath))
				try
				{
					...
					// Apply zoom by scaling the image (zoom > 1 = larger image, zoom < 1 = smaller)
					var zoom = Math.Clamp(entry.BackgroundZoom, 0.5f, 2.0f);
					var scaledWidth = (int)(_bgWidth * zoom);
					var scaledHeight = (int)(_bgHeight * zoom);
		...
		var placeholder = new Image<Rgba32>(_bgWidth, _bgHeight);
		placeholder.Mutate(ctx => ctx.Fill(SixLabors.ImageSharp.Color.Black));
		return placeholder;
	}
	...
	/// </summary>
	private static IPath BuildShardPath(float startAngleDeg, float endAngleDeg)
	{
		var arcAngleSpan = endAngleDeg - startAngleDeg;
		var arcSegments = Math.Max(12, (int)(arcAngleSpan / 3)); // More segments for smoother curves
		...
		var startRad = startAngleDeg * MathF.PI / 180f;
		var innerStart = new SixLabors.ImageSharp.PointF(
			_bgCenterX + _innerRadius * MathF.Cos(startRad),
			_bgCenterY + _innerRadius * MathF.Sin(startRad)
		);
		pathBuilder.MoveTo(innerStart);

		// Line from inner start to outer start (radial edge)
		var outerStart = new SixLabors.ImageSharp.PointF(
			_bgCenterX + _outerRadius * MathF.Cos(startRad),
			_bgCenterY + _outerRadius * MathF.Sin(startRad)
		);
		pathBuilder.LineTo(outerStart);
		...
		for (var i = 1; i <= arcSegments; i++)
		{
			...
			var arcPoint = new SixLabors.ImageSharp.PointF(
				_bgCenterX + _outerRadius * MathF.Cos(rad),
				_bgCenterY + _outerRadius * MathF.Sin(rad)
			);
			pathBuilder.LineTo(arcPoint);
		}
		...
		var endRad = endAngleDeg * MathF.PI / 180f;
		var innerEnd = new SixLabors.ImageSharp.PointF(
			_bgCenterX + _innerRadius * MathF.Cos(endRad),
			_bgCenterY + _innerRadius * MathF.Sin(endRad)
		);
		pathBuilder.LineTo(innerEnd);
		...
		for (var i = arcSegments - 1; i >= 0; i--)
		{
			...
			var arcPoint = new SixLabors.ImageSharp.PointF(
				_bgCenterX + _innerRadius * MathF.Cos(rad),
				_bgCenterY + _innerRadius * MathF.Sin(rad)
			);
			pathBuilder.LineTo(arcPoint);
		}
	...
	/// </summary>
	private static SixLabors.ImageSharp.PointF CalculateShardCenter(float startAngleDeg, float endAngleDeg)
	{
		// The center of the shard is at the middle angle and middle radius
		var midAngleDeg = (startAngleDeg + endAngleDeg) / 2f;
		var midRadius = (_innerRadius + _outerRadius) / 2f;
		var midRad = midAngleDeg * MathF.PI / 180f;

		return new SixLabors.ImageSharp.PointF(
			_bgCenterX + midRadius * MathF.Cos(midRad),
			_bgCenterY + midRadius * MathF.Sin(midRad)
		);
	}
	...
	/// </summary>
	private static void DrawShardOntoCanvas(Image<Rgba32> canvas, Image<Rgba32> shardImage, IPath shardPath,
		SixLabors.ImageSharp.PointF shardCenter, float panX, float panY)
	{
		// Center the (possibly zoomed) image on the shard center
		var offsetX = shardCenter.X - shardImage.Width / 2f;
		var offsetY = shardCenter.Y - shardImage.Height / 2f;
		...
		// Apply pan offset - pan moves the entire image rectangle
		// Pan values are in normalized units, scale to base dimensions
		var panOffsetX = panX * _bgWidth * 0.5f;
		var panOffsetY = panY * _bgHeight * 0.5f;
		offsetX += panOffsetX;
		offsetY += panOffsetY;
	...
	/// </summary>
	private static void DrawShardBorders(Image<Rgba32> canvas, List<IPath> shardPaths)
	{
		var pen = SixLabors.ImageSharp.Drawing.Processing.Pens.Solid(SixLabors.ImageSharp.Color.Black, _borderThickness);
		...
		canvas.Mutate(ctx =>
		{
			...
			var innerCircle = new EllipsePolygon(_bgCenterX, _bgCenterY, _innerRadius);
			ctx.Draw(pen, innerCircle);

			// And the outer circle border
			var outerCircle = new EllipsePolygon(_bgCenterX, _bgCenterY, _outerRadius);
			ctx.Draw(pen, outerCircle);
		});
	}
	...
	/// </summary>
	private static void DrawCenterCircle(Image<Rgba32> canvas, string centerText)
	{
		// Draw filled black circle in center
		var centerCircle = new EllipsePolygon(_bgCenterX, _bgCenterY, _centerCircleRadius);
		...
		if (!string.IsNullOrWhiteSpace(centerText))
		{
			...
			var text = centerText.Length > 3 ? centerText[..3] : centerText;
			...
			var availableRadius = _centerCircleRadius - borderWidth / 2f - padding;
			var availableDiameter = availableRadius * 2f;

			// Auto-size the font to fit within the circle
			var fontSize = 200f; // Start large
			...
			var originX = _bgCenterX - textCenterX;
			var originY = _bgCenterY - textCenterY;
	...
	/// </summary>
	private static void ApplyGlitchEffects(Image<Rgba32> image, float intensity, int seed)
	{
	...
	/// </summary>
	private static void ApplyRgbShift(Image<Rgba32> image, float intensity, Random random)
	{
		// Max shift scales with intensity (0-30 pixels at max)
		var maxShift = (int)(30 * intensity);
		if (maxShift < 1)
			return;
		...
		var redShiftX = random.Next(-maxShift, maxShift + 1);
		var redShiftY = random.Next(-maxShift / 3, maxShift / 3 + 1);
		var blueShiftX = random.Next(-maxShift, maxShift + 1);
		var blueShiftY = random.Next(-maxShift / 3, maxShift / 3 + 1);
		...
		image.ProcessPixelRows(redChannel, blueChannel, (mainAccessor, redAccessor, blueAccessor) =>
		{
			for (var y = 0; y < mainAccessor.Height; y++)
			{
				...
				for (var x = 0; x < mainAccessor.Width; x++)
				{
					ref var pixel = ref mainRow[x];

					// Get shifted red channel
					var redY = Math.Clamp(y + redShiftY, 0, mainAccessor.Height - 1);
					var redX = Math.Clamp(x + redShiftX, 0, mainAccessor.Width - 1);
					var redRow = redAccessor.GetRowSpan(redY);
					var newRed = redRow[redX].R;

					// Get shifted blue channel
					var blueY = Math.Clamp(y + blueShiftY, 0, mainAccessor.Height - 1);
					var blueX = Math.Clamp(x + blueShiftX, 0, mainAccessor.Width - 1);
					var blueRow = blueAccessor.GetRowSpan(blueY);
					var newBlue = blueRow[blueX].B;
	...
	/// </summary>
	private static void ApplyScanlines(Image<Rgba32> image, float intensity)
	{
		// Scanline frequency and darkness scale with intensity
		var scanlineSpacing = Math.Max(2, 6 - (int)(4 * intensity)); // 6 to 2 pixel spacing
		var darkness = 0.3f + 0.5f * intensity; // 30% to 80% dark
	...
	/// </summary>
	private static void ApplyImageDistortion(Image<Rgba32> image, float intensity, Random random)
	{
		// Distortion amplitude scales with intensity (0-40 pixels at max)
		var amplitude = 40 * intensity;
		if (amplitude < 1)
			return;

		// Random wave frequency
		var frequency = 0.005f + (float)random.NextDouble() * 0.015f;
		var phase = (float)random.NextDouble() * MathF.PI * 2;

		using var original = image.Clone();

		image.ProcessPixelRows(original, (destAccessor, srcAccessor) =>
		{
			for (var y = 0; y < destAccessor.Height; y++)
			{
				var destRow = destAccessor.GetRowSpan(y);

				// Calculate wave offset for this row
				var offset = (int)(MathF.Sin(y * frequency + phase) * amplitude);

				for (var x = 0; x < destAccessor.Width; x++)
				{
					var srcX = x + offset;
					...
					if (srcX < 0)
						srcX = 0;
					if (srcX >= srcAccessor.Width)
						srcX = srcAccessor.Width - 1;
	...
	/// </summary>
	private static void ApplyBlockGlitches(Image<Rgba32> image, float intensity, Random random)
	{
		// Number of glitch blocks scales with intensity
		var blockCount = (int)(20 * intensity);
		if (blockCount < 1)
			return;
		...
		for (var i = 0; i < blockCount; i++)
		{
			...
			var blockWidth = random.Next(50, 400);
			var blockHeight = random.Next(5, 50);

			// Random source position
			var srcX = random.Next(0, image.Width - blockWidth);
			var srcY = random.Next(0, image.Height - blockHeight);

			// Random horizontal displacement
			var offsetX = random.Next(-100, 101);
			var destX = Math.Clamp(srcX + offsetX, 0, image.Width - blockWidth);

			// Copy the block with displacement
			image.ProcessPixelRows(original, (destAccessor, srcAccessor) =>
			{
				for (var y = 0; y < blockHeight && srcY + y < srcAccessor.Height; y++)
				{
					var srcRow = srcAccessor.GetRowSpan(srcY + y);
					var destRow = destAccessor.GetRowSpan(srcY + y);

					for (var x = 0;
						 x < blockWidth && srcX + x < srcAccessor.Width && destX + x < destAccessor.Width;
						 x++)
						if (destX + x >= 0)
			...
			{
				var tintR = (byte)(random.NextDouble() < 0.5 ? 255 : 0);
				var tintG = (byte)(random.NextDouble() < 0.5 ? 255 : 0);
				var tintB = (byte)(random.NextDouble() < 0.5 ? 255 : 0);
				var tintStrength = 0.2f + 0.3f * intensity;

				image.ProcessPixelRows(accessor =>
				{
					for (var y = srcY; y < srcY + blockHeight && y < accessor.Height; y++)
					{
						var row = accessor.GetRowSpan(y);
						for (var x = destX; x < destX + blockWidth && x < accessor.Width; x++)
							if (x >= 0)
							{
								ref var pixel = ref row[x];
								pixel = new Rgba32(
	...
	/// </summary>
	private static void ApplyGlitchEffectsScaled(Image<Rgba32> image, float intensity, int seed, float scale)
	{
		var random = new Random(seed);
	...
	/// </summary>
	private static void ApplyRgbShiftScaled(Image<Rgba32> image, float intensity, Random random, float scale)
	{
		// Scale the max shift (original: 30 pixels at 1920px)
		var maxShift = (int)(30 * intensity * scale);
		if (maxShift < 1)
			return;
	...
	/// </summary>
	private static void ApplyScanlinesScaled(Image<Rgba32> image, float intensity, float scale)
	{
		// Scale scanline spacing (original: 6 to 2 pixels at 1920px)
		var baseSpacing = Math.Max(2, 6 - (int)(4 * intensity));
		var scanlineSpacing = Math.Max(1, (int)(baseSpacing * scale));
		var darkness = 0.3f + 0.5f * intensity;
	...
	/// </summary>
	private static void ApplyImageDistortionScaled(Image<Rgba32> image, float intensity, Random random, float scale)
	{
		// Scale amplitude (original: 40 pixels at 1920px)
		var amplitude = 40 * intensity * scale;
		if (amplitude < 1)
			return;

		// Frequency needs to be scaled inversely to work with smaller image
		var frequency = (0.005f + (float)random.NextDouble() * 0.015f) / scale;
		var phase = (float)random.NextDouble() * MathF.PI * 2;
	...
	/// </summary>
	private static void ApplyBlockGlitchesScaled(Image<Rgba32> image, float intensity, Random random, float scale)
	{
		...
		for (var i = 0; i < blockCount; i++)
		{
			...
			var blockWidth = (int)(random.Next(50, 400) * scale);
			var blockHeight = (int)(random.Next(5, 50) * scale);
			...
			if (blockWidth >= image.Width)
				blockWidth = image.Width / 2;
			if (blockHeight >= image.Height)
				blockHeight = image.Height / 4;

			var srcX = random.Next(0, Math.Max(1, image.Width - blockWidth));
			var srcY = random.Next(0, Math.Max(1, image.Height - blockHeight));

			// Scale displacement (original: -100 to 100 at 1920px)
			var maxOffset = (int)(100 * scale);
			var offsetX = random.Next(-maxOffset, maxOffset + 1);
			var destX = Math.Clamp(srcX + offsetX, 0, Math.Max(0, image.Width - blockWidth));
