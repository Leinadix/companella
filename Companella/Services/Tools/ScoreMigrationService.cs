...
public class ScoreMigrationService
{
	...
	private static bool IsOsuRunning()
	{
		...
		if (osuProcesses.Length == 0) osuProcesses = Process.GetProcessesByName("osu");

		foreach (var proc in osuProcesses) proc.Dispose();
	...
	private bool ForceCloseOsu()
	{
		...
		if (!string.IsNullOrEmpty(osuDir)) _osuExePath = Path.Combine(osuDir, "osu!.exe");
	...
	private static bool WaitForOsuToStart(int timeoutSeconds = 30)
	{
	...
	private bool RestartOsuToFlushScores()
	{
		...
		// Start osu!
		if (!StartOsu()) return false;

		// Wait for it to start
		if (!WaitForOsuToStart(30)) return false;
	...
	{
		return MigrateSessionScoresInternal(true);
	}
	...
	private MigrationResult MigrateSessionScoresInternal(bool autoRestart)
	{
		...
		var wasOsuRunning = IsOsuRunning();
		...
		try
		{
			...
			else
			{
				...
				if (!RestartOsuToFlushScores())
					...
					Logger.Info(
						"[ScoreMigration] Warning: Could not restart osu! to flush scores, proceeding anyway...");
			}
		...
		finally
		{
			...
			if (autoRestart && wasOsuRunning) StartOsu();
		}
	...
	public CleanupResult CleanupSessionMaps()
	{
		...
		try
		{
			...
			result.Success = true;
			Logger.Info(
				$"[ScoreMigration] Cleanup complete: {result.FilesDeleted} files deleted, {result.FilesFailed} failed");
		}
		catch (Exception ex)
		{
			result.Error = ex.Message;
	...
	/// </summary>
	private static MigrationResult PerformMigrationCore(string osuDir, string songsDir, string scoresPath)
	{
		var result = new MigrationResult();
		...
		try
		{
			...
			foreach (var (sessionPath, originalPath) in sessionMaps)
			{
				...
				if (!string.IsNullOrEmpty(sessionHash) && !string.IsNullOrEmpty(originalHash))
				{
					...
					Logger.Info(
						$"[ScoreMigration] Mapping: {Path.GetFileName(sessionPath)} -> {Path.GetFileName(originalPath)}");
				}
			}
			...
			foreach (var score in scores)
				if (hashMapping.TryGetValue(score.BeatmapHash, out var originalHash))
				{
					...
					if (!result.MigratedMaps.Contains(score.BeatmapHash)) result.MigratedMaps.Add(score.BeatmapHash);
				}
			...
			foreach (var score in scoresToRemove) scores.Remove(score);
	...
	/// </summary>
	private static List<string> FindAllSessionCopyFiles(string songsDir)
	{
	...
	/// </summary>
	private static List<(string SessionPath, string OriginalPath)> FindSessionCopyMaps(string songsDir)
	{
		...
		foreach (var osuPath in allOsuFiles)
			try
			{
				...
				if (!string.IsNullOrEmpty(originalPath)) results.Add((osuPath, originalPath));
			}
			catch
			{
				// Skip files that can't be parsed
	...
	/// </summary>
	private static string? FindOriginalMap(string sessionPath, OsuFile sessionFile)
	{
...
#pragma warning restore CA5351 // Do Not Use Broken Cryptographic Algorithms
	...
	/// </summary>
	private static (List<OsuScore> Scores, int Version) ReadScoresDb(string scoresPath)
	{
	...
	/// </summary>
	private static void WriteScoresDb(string scoresPath, List<OsuScore> scores, int version)
	{
		...
		foreach (var kvp in scoresByBeatmap)
		{
			...
			foreach (var score in beatmapScores)
			{
				...
				if ((score.Mods & (1 << 23)) != 0) writer.Write(score.TargetPracticeAccuracy);
			}
		}
