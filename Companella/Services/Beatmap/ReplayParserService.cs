...
public class ReplayParserService
{
	...
	public List<string> GetReplayFolders()
	{
		...
		if (folders.Count == 0) Logger.Info($"[ReplayParser] No replay folders found in: {osuDir}");
	...
	public string? FindMostRecentReplay(int maxAgeSeconds = 60)
	{
		...
		try
		{
			...
			var recentFiles = allReplayFiles
				.Select(f => new
				{ File = f, MostRecentTime = f.LastWriteTime > f.CreationTime ? f.LastWriteTime : f.CreationTime })
			...
			if (recentFiles.Count == 0)
			{
				...
				if (newest != null)
					Logger.Info(
						$"[ReplayParser] No recent replay files (max age: {maxAgeSeconds}s). Newest file is {(DateTime.Now - newest.LastWriteTime).TotalSeconds:F1}s old: {newest.Name}");
				else
			...
			Logger.Info(
				$"[ReplayParser] Found recent replay: {mostRecent.File.Name} ({(DateTime.Now - mostRecent.MostRecentTime).TotalSeconds:F1}s ago) in {mostRecent.File.DirectoryName}");
	...
	public static Replay? ParseReplay(string replayPath)
	{
	...
	public static List<ManiaKeyEvent> ExtractManiaKeyEvents(Replay replay, int keyCount = 4)
	{
		...
		if (events.Count > 0)
		{
			...
			if (presses.Count > 0)
				Logger.Info(
					$"[ReplayParser] Press count: {presses.Count}, First press at {presses.First().Time:F0}ms, last at {presses.Last().Time:F0}ms");
		}
	...
	public List<string> FindReplaysForBeatmap(string beatmapPath)
	{
		...
		try
		{
			beatmapHash = File.OpenRead(beatmapPath).Md5();
				...
				foreach (var replayPath in replayFiles)
					try
					{
						...
						if (replay.BeatmapMD5Hash?.ToLowerInvariant() == beatmapHash)
						{
							...
							Logger.Info(
								$"[ReplayParser] Found matching replay: {Path.GetFileName(replayPath)} ({replay.PlayerName})");
						}
					}
					catch
					{
	...
	public Replay? FindReplayByScoreData(ResultsScreenData resultsData, string beatmapHash)
	{
		...
		try
		{
			...
			Logger.Info(
				$"[ReplayParser] Found matching score: {matchingScore.PlayerName}, Timestamp: {matchingScore.Timestamp}");
	...
	/// </summary>
	private static ScoreDbEntry? FindScoreInDatabase(string scoresPath, ResultsScreenData resultsData, string beatmapHash)
	{
		try
		{
			...
			for (var i = 0; i < beatmapCount; i++)
			{
				...
				for (var j = 0; j < scoreCount; j++)
				{
					...
					if (score.BeatmapHash == null) continue;
					// Check if this score matches our criteria
					if (mapHash.ToLowerInvariant().Equals(beatmapHash, StringComparison.OrdinalIgnoreCase) ||
						score.BeatmapHash.Equals(beatmapHash, StringComparison.OrdinalIgnoreCase))
	...
	private Replay? FindReplayByFilename(ScoreDbEntry scoreEntry)
	{
		...
		foreach (var folder in folders)
			try
			{
				...
				{
					Logger.Info(
						$"[ReplayParser] Found {matchingFiles.Length} files matching beatmap hash in {Path.GetFileName(folder)}");
					foreach (var file in matchingFiles) Logger.Info($"[ReplayParser]   - {Path.GetFileName(file)}");
					...
					foreach (var file in matchingFiles)
						try
						{
							...
							var scoreMatches = replay.ReplayScore == scoreEntry.TotalScore;
							var comboMatches = replay.Combo == scoreEntry.MaxCombo;
							var hitsMatch = replay.Count300 == scoreEntry.Count300 &&
											replay.Count100 == scoreEntry.Count100 &&
											replay.Count50 == scoreEntry.Count50 &&
											replay.CountMiss == scoreEntry.CountMiss;

							Logger.Info(
								$"[ReplayParser] Checking {Path.GetFileName(file)}: Score={replay.ReplayScore} ({(scoreMatches ? "OK" : "X")}), Combo={replay.Combo} ({(comboMatches ? "OK" : "X")}), Hits={replay.Count300}/{replay.Count100}/{replay.Count50}/{replay.CountMiss} ({(hitsMatch ? "OK" : "X")})");
					...
					Logger.Info(
						$"[ReplayParser] No exact metadata match found among {matchingFiles.Length} candidates");
				}
			}
			catch (Exception ex)
			{
	...
	/// </summary>
	private static ScoreDbEntry ReadScore(BinaryReader reader)
	{
	...
	/// </summary>
	public static string? GetBeatmapHash(string beatmapPath)
	{
		if (!File.Exists(beatmapPath))
			return null;
		...
		try
		{
			return File.OpenRead(beatmapPath).Md5();
		}
		catch
		{
			return null;
	...
	public static float GetRateFromMods(Replay replay)
	{
		...
		if ((mods & MOD_NIGHTCORE) != 0 || (mods & MOD_DOUBLE_TIME) != 0) return 1.5f;

		if ((mods & MOD_HALF_TIME) != 0) return 0.75f;
	...
	public static bool HasMirrorMod(Replay replay)
	{
		var mods = (int)replay.Mods;
	...
	public string? FindBeatmapByHash(string beatmapMd5Hash)
	{
		...
		try
		{
			...
			foreach (var osuFile in osuFiles)
				try
				{
					var fileHash = File.OpenRead(osuFile).Md5();

					if (fileHash.Equals(beatmapMd5Hash, StringComparison.OrdinalIgnoreCase))
					{
