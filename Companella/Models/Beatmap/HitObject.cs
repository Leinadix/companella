		...
		try
		{
			...
			if (isHold && parts.Length >= 6)
			{
				...
				if (colonIndex >= 0)
				{
					...
					if (double.TryParse(endTimePart, NumberStyles.Float, CultureInfo.InvariantCulture,
							out var parsedEndTime))
						endTime = parsedEndTime;

					hitSample = lastPart.Substring(colonIndex + 1);
				}
				else if (double.TryParse(lastPart, NumberStyles.Float, CultureInfo.InvariantCulture,
							 out var parsedEndTime))
				{
					endTime = parsedEndTime;
				}
