...
public class MsdControlPoint
{
	...
	/// </summary>
	public MsdControlPoint Clone()
	{
		return new MsdControlPoint(TimePercent, Msd, Skillset);
	}
}
...
public class MsdCurveConfig
{
	...
	public MsdCurveConfig Clone()
	{
		...
		clone._points.Clear();
		foreach (var point in _points) clone._points.Add(point.Clone());
	...
	public static MsdCurveConfig? GenerateFromTrends(SkillsTrendResult trends,
		SessionGenerationMode mode = SessionGenerationMode.Normal, int sessionDurationMinutes = 80)
	{
		...
		if (mode == SessionGenerationMode.Acc) return GenerateAccMode(trends, config, sessionDurationMinutes);
	...
	/// </summary>
	private static MsdCurveConfig? GenerateAccMode(SkillsTrendResult trends, MsdCurveConfig config,
